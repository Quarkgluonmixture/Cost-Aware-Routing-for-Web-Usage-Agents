...
> **Substance-lock anchor (post Q3=A doctrine fix, commit c741c3a 2026-05-18 evening)**: Git SHA `c741c3af40e3a070cdbd5a199bead0bd0e2b1064` + Git tag `preregistration-locked-doctrine-fix-q3a-20260518T195209Z`. Body-text references to "Git tag `preregistration-locked` at SHA `ef609a3`" below describe the pre-doctrine-fix substance-lock (retained on origin for git-history continuity); the current canonical substance-lock anchor at `c741c3af` includes the B-1750~B-1759 doctrine fixes from commit c741c3a (DOI 1 timing decoupled from data landing + H10 ↔ R-rule decoupling + H10 router architecture E'' alignment + δ=1.0pp demoted for H10 + B2 cell-count taxonomy + H3 FE-pool vs Holm primary/transparency split + H9/H11 paper-2 forward-stub qualifier).
...
| **OSF DOI 1 — Pre-canonical-outcome-creation witness for Fire-3** (paper-1 cryptographic external witness; supersedes advisor email per B-1570 + B-1650; supersedes fire-2-era witness per B-1670 retraction) | OSF public registration submitted **at Fire-3 PID-alive moment** (5-min window before first episode-summary creation), captures canonical pre-outcome anchor via `artifact_existence_check_doi1_canonical_<fire3-UTC-TS>.txt` (canonical schema per `p79/experiment/logger_v2.py:111` + `analysis.py:209`); submission timestamp = cryptographic anchor; DOI string auto-assigned 0-48h post-submission via OSF admin approval (default per help.osf.io/article/330). Bundle = preregistration.md + osf_lock_manifest.md + locked_versions.md + model/dataset/ethics/evaluator/compute/neurips/negative-results/release-redaction/topvenue + env_snapshot.json (fire-3 epoch) + paper_drafts (frozen pre-outcome) + DOI_1_README.md + canonical witness + interim retraction witness (audit-trail bridge) | ⏳ Layer 3 PENDING Fire-3 launch (post NLTK punkt evaluator substrate fix in parallel session); witness pattern retraction landed 2026-05-18 ~14:45 UTC per B-1670; canonical capture deferred to Fire-3 PID-alive |
