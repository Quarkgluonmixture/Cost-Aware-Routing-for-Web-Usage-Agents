...
|---|---|---|
| **VWA submodule SHA** | **`2c15d66d120f8498633ae65057aa50a34b3e93e0`** (branch `p79-patches`, Fire-6 RCA Stage C1b) | `git -C external/visualwebarena rev-parse HEAD` (re-locked 2026-05-20 for C1b screenshot-timeout non-fatal recovery, 12th commit; supersedes 2026-05-18 DOI1 lock at `ac33d2f`, supersedes 2026-05-17 A1.25 GRL Chunk 4 lock at `1c3a615`) |
...
| **`p79-patches` ↔ upstream diff SHA-256** *(RETIRED 2026-05-20, B-1781)* | ~~`20921a57c79c469c914476ecfc33ae92364880e1f2d513a5d1355accf0d59248`~~ — **STALE** (computed at the 9-commit/pre-C1b tree; HEAD is now the 12-commit `2c15d66`, actual = `894e5afad5252f5496395417487374fb0b36e8570f7a99a2b14de9b875f5228b`). **NOT an enforced witness.** | /stress GRL audit B-1781 (codex Mode B): B-1774 resynced the tree-hash chain across 6 locations but NOT this derivable diff-SHA → stale + ungated = provenance theater. **Retired** — the **submodule SHA + tree-hash chain are the sole enforced reproducibility witnesses** (the diff is derivable on demand: `git -C external/visualwebarena diff 89f5af2..HEAD \| sha256sum`). |
| **Branch commit list** | `e9c63b7` (networkidle pre-screenshot, superseded by `eb5cbd8` single-barrier fix) · `3f9ceca` (composite runtime patches: viewport ratio fix + Tailscale routing + NumPy 2.0 compat + `VWA_EVAL_MODEL` env var + lazy OpenAI client init + `Meta+A` clear-before-type extension) · `16b60d7` (setup script + WA non-visual task configs + VWA shopping pool) · `832f037` (`.gitignore` runtime data) · `f0c835b` (B-91 empty-pred LLM-judge guard) · `eb5cbd8` (/stress A1.18 full sweep — 913 task config IP→placeholder, envs.py env-driven launch args, networkidle single-barrier, tightened assert+log, lazy OpenAI lock+env-hash, async response-shape normalize, aexecute_action signature fix, create_upload_action UPLOAD enum, async float() cast, prepare.sh Windows fallback) · `c1765ee` (/stress A1.25 GRL Chunk 1 — B-445 create_mouse_click truthiness fix, B-446 sync SELECT_OPTION args forward, B-447 sync UPLOAD parser+factory) · `1c3a615` (/stress A1.25 GRL Chunk 4 — B-535 llm_fuzzy_match + llm_ua_match polarity inversion, B-538 async SELECT_OPTION sibling, B-539 UPLOAD field decouple from key encoding + remove \n enter-flag, B-540 shlex.split VWA_CHROMIUM_LAUNCH_ARGS) · `2f9b0b4` (/stress A1.18-re Chunk 1 — 11-fix substrate sweep B-577+B-582~B-588+B-591+B-596+B-598: TOCTOU + polarity in helper_functions.py / fail-loud + BadRequest in openai_utils.py / UPLOAD serialize + hover contract in actions.py / image timeout + networkidle observability + PlaywrightTimeoutError import in processors.py / idempotency + JSON det + manifest in generate_test_data.py / PY argv array in prepare.sh) · `f883a11` (/stress 深入审 followup Mode A fire-event lock 2026-05-18, 10th commit — `git rm --cached config_files/vwa/test_shopping.json` to untrack URL-substituted generated file) · **`ac33d2f`** (11th commit, complement to f883a11 same logical fix split across 2 commits due to staging oversight — `.gitignore` L144 typo correction `config_files/vwatest_shopping.json` → `config_files/vwa/test_shopping.json` (missing `/` slash) + add `config_files/generation_manifest.json` entry; combined effect restores 3-site symmetry across test_<site>.json deployment-derived files matching test_classifieds.json + test_reddit.json already gitignored correctly; **zero substantive content change** across both commits) · **`2c15d66`** (12th commit, Fire-6 RCA Stage C1b 2026-05-20 — screenshot-timeout non-fatal recovery in `AsyncScriptBrowserEnv.astep`: heavy-DOM-page `Page.screenshot` timeout returns blank ndarray + `info['screenshot_timeout_recovered']=True` instead of raising; `content()` captured first; mode-AGNOSTIC at submodule, dom-only vs som/vision fatality gating lives in P79 `VWAWrapper._gate_screenshot_timeout`; behavioural impact = dom artifact-only screenshot timeout no longer quarantines (Fire-4 task-75 agent_observation class), som/vision unchanged, screenshot still attempted every step → latency comparability preserved) | `git -C external/visualwebarena log --reverse --format="%h %s" 89f5af2..HEAD` |
...
This will:
1. Check `git -C external/visualwebarena rev-parse HEAD` matches `2c15d66d120f8498633ae65057aa50a34b3e93e0` (branch `p79-patches`)
...
- **2026-05-20 (Fire-6 RCA Stage C1b)**: VWA submodule SHA pin updated from
  `ac33d2fcd9cec2fcbeddd56d0fa3da58b4c7e927` (DOI1 lock) → `2c15d66d120f8498633ae65057aa50a34b3e93e0`
  (12th commit: screenshot-timeout non-fatal recovery in `AsyncScriptBrowserEnv.astep`). Tree-hash
  chain `752caeb…` → `2696d0a61e2f70536f247ebb225f51c262b657d8b8b7b407f8581b75757a8bae`. Synced across
  all enforcement points: `snapshot_env.py` (HEAD + chain consts), `preflight_v2.sh:413`, `Makefile`
  (`pre-launch-check` + `pre-release-check` LOCK_SHA + LOCK_CHAIN), `preregistration.md §7`, this file
  (SHA + commit list + check), `section4_limitations_disclosure.md §4.X.11` (+ backfilled the
  pre-existing missing `f883a11` / `ac33d2f` table rows). **OSF implication**: supersedes the DOI1
  (2026-05-18) submodule lock — Fire-6 data is produced at `2c15d66`, so a fresh pre-run deposit
  (DOI2) is required before/at Fire-6 to keep the OSF SBOM matching the code that produced the data.
- **2026-05-18 (B-1400 /stress A2.7 P0-1-B* codex Mode B OOB)**: VWA submodule SHA pin updated from
  `1c3a615308fd9f17c73a9d33a96cf29ec6807d48` (A1.25 GRL Chunk 4) → `ac33d2fcd9cec2fcbeddd56d0fa3da58b4c7e927`
  (A1.18-re Chunk 1, 11-fix substrate sweep). Pre-fix split-brain: `preflight_v2.sh:413`, `Makefile LOCK_SHA`,
  `paper §4.X.11`, and `preregistration §7` had all been bumped to `2f9b0b4` post-A1.18-re Chunk 2, but
  `scripts/provenance/snapshot_env.py:184 VWA_LOCKED_HEAD_SHA` + this `locked_versions.md` table were missed
  in the sibling-propagation pass. Result: orchestrator Gate 2/3/4 passed (HEAD = `2f9b0b4`) while runner
  provenance capture raised `_critical_error` under `P79_PAPER_GRADE=1` — launch path death at first cell.
  Tree-hash chain at `snapshot_env.py:186` also recomputed for the new 9-commit chain
  (`3e88135198bcee988a5e14469637ffd286842e7cb5dae758d3ae5ccb4320cd8a`). diff SHA-256 also recomputed
  (`20921a57c79c469c914476ecfc33ae92364880e1f2d513a5d1355accf0d59248`). 9th commit added: `2f9b0b4`
  (B-577+B-582~B-588+B-591+B-596+B-598 substrate sweep — see Branch commit list row).
