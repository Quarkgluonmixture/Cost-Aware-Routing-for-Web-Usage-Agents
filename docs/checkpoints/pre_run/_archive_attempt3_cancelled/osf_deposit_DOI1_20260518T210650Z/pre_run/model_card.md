...
- **⚠️ B0 vs B1/B2 deployment-stack confounder (B-127 disclosure 2026-05-15 per gemini Mode C P2-4; **B-1206 update 2026-05-18 post-B-991 retire**)**: The B0 (API) vs B1/B2 (local) split is a deployment-stack confounder for any B0-vs-B1/B2 SR comparison (observed SR differences mix capability-scale with deployment-stack). **Archive-era only**: B0 had a GLM-5.1 parse-error rescue scaffold (`proxy_api_agent.py::_call_glm_extract`, enabled by `use_glm_fallback: true`) that converted malformed JSON responses into valid actions — historical rescue rate ~1.49% (笔记 §95). **Phase 1a clean substrate (B-991 retire 2026-05-17 + A2.3b cleanup 2026-05-18)**: GLM rescue physically deleted (~155 LOC removed from `proxy_api_agent.py`); `_call_glm_extract` no longer exists; `use_glm_fallback: true` now raises `RuntimeError` at construction. B0 paper-grade fires use native AWS proxy `tool_choice="auto"` + `body["tool_calls"][0].function.arguments` parse path (B-991) + 1-element-list `element_id` coerce (B-1201) + paper-grade missing-logprob fail-loud (B-1203). Cross-baseline parse contract: all three baselines (B0/B1/B2) now emit structured action JSON via the same `validate_action_detailed` taxonomy without rescue scaffolds. Scale claims in paper §1 remain softened per the deployment-stack confounder, but **the GLM-specific component of that confounder is retired** in Phase 1a clean-substrate data. (See also section3 §3.5.1 cross-baseline disclosure + 实验笔记 §210 + master_bug_catalog A1.2-followup section.)
