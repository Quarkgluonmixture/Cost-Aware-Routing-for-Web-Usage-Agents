...
**Critical epistemic distinction (paper rigor signal)**:
- **Artifact existence**: industry already deploys flat-text routing configurations
- **Research characterization**: nobody has run controlled DOM-vs-phantom comparison on identical task pool to isolate per-dimension routing-behavior effect
- These are different epistemic levels — industry deployment ≠ scientific finding
- Paper claim is at characterization level, not artifact-creation level
- Reviewer attack "industry already does this" rebutted by "industry does deployment, not characterization"
