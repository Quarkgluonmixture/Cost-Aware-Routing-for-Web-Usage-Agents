...
>
> **Last updated**: 2026-05-04 deepest evening (§21 fact-check correction — prior "interactive-only filter / P79 preserve all elements" over-claim 撤回 after reading `external/visualwebarena/browser_env/processors.py:513-619`. Verified actual: VWA's `parse_accessibility_tree` + `clean_accesibility_tree` produces a11y-tree-roled output with role-based empty-node drop + StaticText dedup — **scope similar to industry SDK** Playwright MCP / agent-browser. Token gap is format-axis: P79 emits URL property + a11y properties + tab-indented hierarchy (~1000-1500 snapshot tokens), industry SDK trims compact (~200-400). §1 hook table "3008 tokens" 是 full prompt context not observation alone. Two orthogonal compression axes: substitution mechanism (paper main) + format-axis trim (industry deployment) — both INDEPENDENT of element-scope axis (similar across us/industry).)
>
...
| **Playwright MCP** (Microsoft) + **agent-browser** (Vercel Labs) | Convergent industry design: ~200-400 tokens per snapshot, accessibility-tree-based ref system (`[ref=eN]` / `@eN`), integrated with Claude Code / Cursor / Codex / Gemini agent CLIs. Production deployment SDKs at (ii)×L2; we characterize the (ii)×L3 inference-time substitution that lies above this layer, on same Qwen3-VL family. **Format-axis orthogonal** (verified by reading processors.py:513-619 2026-05-04): scope similar (both a11y-tree-roled), gap from format choices — P79 includes URL property + a11y properties + tab-indented hierarchy (~1000-1500 tokens snapshot); industry SDK trims to compact ref-only format (~200-400 tokens). Format trims are stackable industry optimizations independent of substitution-axis claim. |
