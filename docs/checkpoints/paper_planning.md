...
>
> **Last updated**: 2026-05-04 deep evening (§21 (ii)×L2 industry sweep extension + token economy filter-scope distinction — agent-browser Vercel 81+ releases dual-mode CDP-direct; Tarsier Reworkd typed brackets + "unimodal text beats GPT-4V + Tarsier-Screenshot 10-20%" closest research precedent; Playwright MCP convergent ~200-400 token design point; Stagehand v3 + Browser Use SDK + Skyvern + Anchor Browser + OpenClaw 361K stars at (ii)×L2; filter-scope orthogonality discovery — industry SDK interactive-only filter ~200-400 tokens vs P79 all-elements ~3000 tokens per §96 ablation control rationale; substitution axis vs filter axis orthogonal — paper §3 method 必 disclose)
>
...
|---|---|---|---|---|
| **Tarsier** (Reworkd v0.6.0 Jun 2024) ⭐ closest research precedent | Playwright | Typed SoM brackets `[#23]` input / `[@23]` link / `[$23]` button + text "ASCII art" for non-vision LLM | (not disclosed) | Internal benchmark claims **"unimodal text beats GPT-4V + Tarsier-Screenshot by 10-20%"** — direct industry analog of phantom routing thesis but **no peer-reviewed systematic characterization**, paper §1/§2 explicit cite |
| **Playwright MCP** (Microsoft) | Playwright | A11y tree text snapshot, refs `[ref=e5]`, **incremental snapshots default** (only deltas) | **200-400 tokens per snapshot** | Convergent industry design, paper §1 cite as canonical (ii)×L2 pipeline preprocessing instance |
| **agent-browser** (Vercel Labs, 81+ releases, latest v0.26.0 April 2026) | **CDP direct** Rust daemon (skip Playwright) | Dual-mode: text refs `@eN` OR SoM annotated image `[N]`; integrated with Claude Code / Cursor / Codex / Gemini / opencode | **200-400 tokens per snapshot** | Production-deployment with widest agent CLI integration; CDP-direct engineering choice (lower latency than Playwright) |
...
**Convergent industry design point**: agent-browser + Playwright MCP both report ~200-400 tokens per snapshot via interactive-only filter + ref-based flat list — **10-15× compression vs full DOM 3000-5000**. **Tarsier directly claims text-only beats text+vision by 10-20%** on internal benchmarks.

**Critical filter-scope distinction (paper §3 method 必须 disclose)**: Industry SDKs (Playwright MCP / agent-browser / Tarsier / Stagehand) all filter to **interactive-only elements** for deployment token economy. P79 paper preserves **all elements** per §96 design (`[id=N] role 'label'` flat list with StaticText / heading / image inclusive, ~3000+ tokens, ±7% of DOM AXTree text per cls/red 实测). **Filter-scope is orthogonal axis to substitution-mechanism axis** — paper isolates substitution-axis capability with high-info-density preserved; industry combines substitution + interactive-only filter for deployment compression. Practitioners can stack interactive-only filter on top of phantom routing space conclusions for industry-grade compression. See §21.6 cost anchor table for full token-economy comparison.
...
**Paper §1 hook 候选 framing** (基于 substitution gradient niche, **Round-3 verified specifics + late-evening industry sweep with 11+ instances at (ii)×L2**):

> "Web agents face a substitution gradient between visual affordance perception and textual surrogate processing. Industry has populated nearly every cell of this gradient: **server-side native emission** (NLWeb, Microsoft, May 2025; deployed at Tripadvisor and Shopify via `/ask` + `/mcp` endpoints emitting schema.org JSON); **pipeline-side preprocessing** (Playwright MCP from Microsoft and agent-browser from Vercel Labs convergently emit ~200-400 token accessibility-tree snapshots with ref identifiers, integrated with Claude Code, Cursor, Codex, and Gemini agent CLIs; OmniParser-v2 outputs structured-points sequences via Two-Stage SPOT prompting; Tarsier from Reworkd 2024 provides typed-bracket SoM annotations and reports unimodal text-only beats GPT-4V + visual SoM by 10-20% on internal benchmarks; Stagehand from Browserbase + Skyvern + Browser Use SDK each provide production-grade accessibility-tree extraction); **pretraining-time LLM-internal grounding** (Magma, Microsoft + UMD + UW Feb 2025; integrates Set-of-Mark + Trace-of-Mark annotations into Qwen3-VL pretraining); **fine-tuning-time substitution** (ScribeAgent, CMU Dec 2024; achieves WebArena 51.3% by fine-tuning Qwen 7B on 6 billion tokens of human demonstration workflows); **RAG-time deploy substitution** (AppAgent-v2, Tencent; offline exploration generates view-state-indexed JSON document, deployment retrieves via cosine similarity); and **pure visual VLM grounding** (UI-TARS, ByteDance Jan 2025; pure-pixel coordinate grounding with System-2 CoT reasoning across hundreds of training VMs). One cell remains unfilled: **inference-time LLM-internal substitution requiring no pretraining, no fine-tuning, no retrieval, no offline exploration, no environment cooperation, no script-side preprocessing**. We characterize this **phantom routing space**: a class of inference-time agent-compute substitutions of pipeline-side visual affordance synthesis (canonical: SoM marker overlay), recovering most routing benefit at near-zero incremental cost on **non-fine-tuned Qwen3-VL** (the same backbone Magma pretrains and ScribeAgent fine-tunes). Although Tarsier's industry deployment offers anecdotal text-beats-vision evidence at ~10-20% margin, no peer-reviewed work systematically characterizes when, where, and how much routing benefit transfers from visual SoM to textual surrogate. P-SoM (cube center, axis 1+2 compound) is the deployment hero satisfying a 4-fold drop-in property: cost ≈ DOM, latency ~50% lower, signal AUROC ≥ baseline, drop-one ≥ 1pp pre-registered. The phantom space exhibits 2-axis empirical structure (axis 1 text payload via P-text; axis 2 SoM-style prompt via P-prompt) — both dimensions contribute non-overlapping unique tasks not solvable by P-SoM alone, evidencing phantom space is a multi-region 2-D structure rather than a collapsed point. **Filter-scope orthogonality**: industry SDKs combine textual substitution with interactive-only element filtering for ~10-15× DOM compression; we preserve all accessibility-tree elements (~3000 tokens, ±7% of DOM text per §96 design rationale) for clean ablation control. Filter-scope optimization can be stacked on phantom routing conclusions for industry-grade deployment compression."

(这版 hook ~520 词, 含 (a) substitution gradient contextualization with **11-instance industry sweep**, (b) **Tarsier closest research precedent** with text-beats-vision claim, (c) Magma+ScribeAgent same-Qwen-base differentiator, (d) phantom space 主 claim, (e) 2-axis structural decomposition, (f) **filter-scope orthogonality disclosure** for paper §3 method rigor)
...
| **Tarsier** (Reworkd 2024) ⭐ closest research precedent | They provide typed SoM brackets `[#23]`/`[@23]`/`[$23]` + text "ASCII art" mode for non-vision LLM, and report internal benchmark "unimodal text-only beats GPT-4V + visual SoM by 10-20%". This is **anecdotal industry analog of phantom routing thesis**, BUT **no peer-reviewed systematic characterization** of when/where/how-much benefit transfers, no controlled experimental design, no cross-cell generalization analysis. Our paper provides community-needed scientific characterization. |
| **Playwright MCP** (Microsoft) + **agent-browser** (Vercel Labs) | Convergent industry design: ~200-400 tokens per snapshot, accessibility-tree-based ref system (`[ref=eN]` / `@eN`), integrated with Claude Code / Cursor / Codex / Gemini agent CLIs. Production deployment SDKs at (ii)×L2; we characterize the (ii)×L3 inference-time substitution that lies above this layer, on same Qwen3-VL family. Filter-scope orthogonal: SDKs filter to interactive-only, we preserve all elements per §96 ablation control. |
| **Stagehand** (Browserbase) + **Browser Use SDK** + **Skyvern** | Production-grade pipeline preprocessing SDKs with hardcoded environmental patches (popup, cookie, dropdown handling); MCP-integrated. We test inference-time LLM substitution of these script-side patches on Qwen3-VL without environmental cooperation. |
| **Anchor Browser** ($6M seed, Cloudflare/Coinbase/Groq partner) | Production infrastructure layer for "millions of agents" via MCP; agent-environment integration plumbing. Our paper is research-level characterization complementing infrastructure deployment. |
| **OpenClaw** (361K stars by early 2026, "fastest-growing OSS in history") | Consumer-scale AI agent platform with browser as one of many tools (browser/canvas/cron/Discord/Slack); CDP + MCP. Paper §1 motivation context: agent browser is now consumer-mainstream, paper-level characterization timely. |
...
| **Industry SDK convergent token economy at ~200-400 per snapshot** ✅ verified Vercel agent-browser + Microsoft Playwright MCP | agent-browser README + Playwright MCP docs | Both Vercel agent-browser and Playwright MCP independently converge on **~200-400 tokens per page snapshot via interactive-only filter + ref-based flat list** = 10-15× compression vs full DOM 3000-5000 tokens. **Paper §3 method footnote**: industry SDK compression combines (a) substitution (visual → text), and (b) interactive-only filter (orthogonal to substitution axis). |
...
| **Filter-scope orthogonality (paper §3 method 必备 disclosure)** | Convergent observation across agent-browser, Playwright MCP, Tarsier, Stagehand 实测 + 我们 §96 design | **Industry SDKs filter to interactive-only elements** (~200-400 tokens) for deployment economy; **P79 paper preserves ALL elements** (~3000 tokens, ±7% of DOM AXTree text per cls/red 实测) per §96 design rationale (ablation control variable). **Two orthogonal compression axes**: (1) substitution mechanism (visual marker render → textual ref list) is paper-1 main characterization axis; (2) interactive-only filtering is industry deployment optimization stackable on top. Paper §1/§3 explicit論證 paper isolates (1), industry combines (1)+(2). Practitioners can apply (2) on top of paper-1 conclusions for deployment compression. **Hidden paper-strategic upside**: phantom routing space proven at 3000+ token full-information density (more stringent test than industry interactive-only-filtered 200-400 tokens). |
