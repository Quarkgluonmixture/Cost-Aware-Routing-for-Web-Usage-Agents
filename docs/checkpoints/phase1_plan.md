...
- [x] **A1.12** `tests/` — pytest 覆盖度 + invariant 测试是否对得上当前实现 (2026-05-16: 17 findings B-342~B-358, §166; suite 333 → 398 GREEN; **B-352 learned router test DEFER** per T1-4=B. **2026-05-17 cold-start re-audit** §187: 3-AI cycle 18 unified findings → 8 fixes B-662~B-669, suite 428 → 467 GREEN, P0-1 source-grep theater explicit-decline per user Q3=C "不 disclose 假定 reviewer 不深读". Closes 2026-05-16 B-352 deferred learned router via B-668; adds B-663 tree-hash chain witness + B-664 SHA SoT + B-665 try/except hoist + B-666 schema snapshot + B-667 real-LLM smoke + B-669 cli smoke.)
