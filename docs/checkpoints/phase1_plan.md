...
- [x] **A1.13** `scripts/queues/queue_{baseline,phantom_*,chain}.sh` — launch + 3-way collision (2026-05-16: combined batch with A1.14, commit `3cd23f2`, §157, B-230~B-236; key fixes = B-224 auth gate hard-fail propagation + Bug 2 URL-locality preflight + RUN_ID nanos+PID+RANDOM collision defense + sibling-propagation defect class closed via `_lib_paper_grade_gates.sh` extraction). **2026-05-17 RE-AUDIT** (post-parallel-session work): 3-AI cycle (Mode A Claude 7 / Mode B codex 8 / Mode C gemini 10 = 25 attack vectors → 19 unique fixes) across 5 commits `57756a3` / `f02bef3` / `92b4598` / `f2a0948` / `6808c56`, B-596~B-614. User v7.7 triaged Q&A: Q1=phantom_dom archive-only canonical sweep (no alias) / Q3=sentinel 100% exact paper-grade (Option A, NOT 90% — empirically history shows 23/24 cells were exact) / Q5=wait-all fix-scope / Q6=WA reset+auth full impl path (B-613 scaffold landed; A100 WA Docker verification deferred follow-up). Key fixes = B-596 phantom_text COND_ID canonical sweep (codex F1 unique OOB) + B-597 WA SITE_EXPECTED_N structural parse (codex+gemini 2-AI OOB) + B-598 `${cond_id!r}` bash bad substitution (Claude OOB) + B-599 `set -e` silent exit (Claude OOB) + B-600 TS_FULL log collision (gemini G4) + B-601 sentinel 100% exact + post-exclusion scored_task_count (codex F5 OOB) + B-603 pgrep regex anchor (Claude+gemini 2-AI) + B-604 B2 gallery alias generalization (Claude+codex 2-AI) + B-605 AUTH_GATE_BYPASS audit log + P79_PAPER_GRADE forbid (codex+gemini 2-AI OOB) + B-608 reset/auth phase timeout wrappers (codex F8 OOB) + B-609 empty URL fail-loud (Claude OOB) + B-610 ntfy env honor 4 queue scripts (gemini G5) + B-611 reset_and_auth_gate named args (gemini G9) + B-612 flock TOCTOU per-(site,benchmark) (codex F4 OOB) + B-613 WA reset+auth scaffold (codex+gemini 2-AI OOB). §183.
