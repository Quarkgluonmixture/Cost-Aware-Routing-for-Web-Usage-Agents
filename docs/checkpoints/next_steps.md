...
| 🔴 **B-1860** | **Qwen 0-1000 坐标范式 vs P79 [0,1]/viewport 栈不匹配 → vision parse_error 13.6% + 48% ep cap-killed (R3671 /diag, 笔记 §285)**。Qwen3-VL 原生 0-1000 坐标, P79 栈全假设 [0,1]/viewport-pixel (prompt + `validate_action_detailed` + `vwa_wrapper.py:614/616` `/1280,720`) → ① 0-1000 标 normalized → validate reject (507 parse error=13.6%) ② 0-1000 标 pixel → wrapper 错归一化 (104 misclick 藏 SR)。铁证 y_max=972>720 viewport + `422=0.422×1000`。**vision SR 13.84% 非 clean (coordinate-scaffold 主导: 48% ep≥3pe/99% fail/cap 3-5 提前处刑), 不可跨 mode 比 SR / 不能只 disclosure**。Fix (**救 format 层不救 grounding 层**): contract=Qwen 0-1000 + wrapper `/1000` 映射 + validate[0,1000]+legacy[0,1] + prompt 改 0-1000 删 coordinate_type + cap 只在归一化后计数。**时机关键**: B0 vision 已污染(重跑); B1/B2 vision 本 fire 未跑 → **改 contract 应在 B1 vision 跑前** (否则一起污染又重跑)。**Per-model**: B0 铁证 / **B1 probe DONE** (DGX 角落 probe: 混用 [0,1]+0-1000; 底部项 (728,**920**) 920>720 排除 pixel = 0-1000 同 B0) / B2 Gemma 待 probe (B2 vision 前 A100)。**逐维度 contract: ≤1.1→[0,1] / >1.1→/1000。fix code DONE** (branch `fix-coordinate-contract-b1860` @3dd343a, 7 files/**1302 tests pass**; 4 wrapper site + select_option bug + validate by-value)。R3671 已 **archived** (_archive_b1860coord_R3671_vision_20260524, verdict completed_ok 3→2)。**apply 流程 (B1 vision 前)**: push fix → A100 pull + witness (tag+OSF amendment) + vision 重跑。follow-up 非block: _infer_coordinate_type label / >1000 upper-bound。= estimand change → witness+amendment + R3671 vision 标 non-canonical + 重跑 vision (+ som coord fallback 统一; dom/P-text/P-prompt 不碰)。 | probe ~0.5h + fix 2-3h + 重跑 vision (B0~8h +B1/B2) | **gate-blocking** — contract+witness+重跑; fire 跑中 p79 immutable, **B1 vision 前决策窗口** |
