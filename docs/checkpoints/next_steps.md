...
| 🟡 **Repro-replicate** | **Run-to-run SR 方差机制 (深挖闭环, 笔记 §282) → paper §4 reproducibility disclosure**。意外副产品 R31194 vs R9755 (两完整 B0 dom 224-run): 聚合 SR 复现 **0.4pp**, per-task agreement 88.8%, discordant 对称 (McNemar p≈1.0) → per-task churn 对称抵消、聚合 SR 稳。**根因 (实证)**: VWA element-ID 标号非确定 (`processors.py:532` CDP/树序号, **B-1858**) → 同页面 obs byte-diff (DOM 91%/SOM 82% step-0), **仅 `[id=N]` 变** (content diff 证结构稳); temp=0 模型对 ID token 敏感 → step-0 action churn。**截图=视觉锚**: SOM step-0 action 90% 稳 > DOM 73% (N=30) → 丢图反而更敏感 (**撤回**"phantom 丢图更可复现", 方向可能反)。**Forward**: (a) paper §4 = "单次 SR 稳吗" defense (聚合 0.4pp + per-task churn 对称); (b) **post-fire**: replay 实验切 ①模型-serving vs ②obs-churn (固定 obs 调 proxy N 次, 避 fire proxy 干扰) + ID-stability 修 (B-1858); (c) **data-gated**: R9725 som 大 N redo matched + phantom 落地测 step-0 action 分叉率确认"图=锚"方向。 | (a) 30min · (b)(c) post-fire/data-gated | paper-write / post-fire/data-gated |
