...
> - 过去 chronicle → [[实验笔记]] (latest §322, 2026-06-07 — §317-322 B1 cls cross-mode /diag discover: som finish-less/dom 真卡死/vision **router-crux REFINE**/ptext+psom **axis-2 P4 phantom-family-wide RESOLVED**/psom **P33 4B>235B gap**; §319 lit-digest)
...
> 🔥 **2026-06-06→07: B1 cls cross-mode /diag discover (5/6 mode) + B-1869 + 2 类 routing-rescuable 证据** ⭐ 最新。fire 健康自走: B0 cls 6 mode 全完 → **B1 cls fire 6-mode 全完** (dom/som/vision/ptext/psom/**pprompt R32516 完成 2026-06-07**) → chain 进 B2 cls (**fire 死活/进度跑 ①② 拿 live**, 别信本行秒级 stale)。本 session **5 condition /diag discover** (**仅 discover 不落码**, ruleset 仍 `5-domsomvispsom-b1860coord`): §317 som (P31=finish-less artifact) · §318 dom (真卡死 + P31 跨模式 confound) · §320 vision (**router crux REFINE**: vision 只救 "AXTree 丢失的已渲染文字" 型 P5 [task40 dom30→vision4 flip 截图确认], 救不了导航/推理型) · §321 ptext (**axis-2 P4 root-ref 瞎猜**) · §322 psom (HERO; **axis-2 P4 phantom-family-wide RESOLVED** — observation 实测 RootWebArea=[2] 无[1] → element_id=1=**幻觉 low-default 非 renumber-root**, 解 §321 两难, §290 第5次; psom>ptext 因 SoM-prompt click-priming; **P33 img-href→PNG 幻觉 B1(4B) 6/7 比 B0(235B) 显著差** = 4B 自纠错 gap)。**2 类干净 routing-rescuable failure** (paper router 硬通货): ① text-in-image rescue (§320 task40) ② attribute-verification deadlock (§321 task17)。**P31 finish-less 跨 5 mode confound 闭环** (som/dom/vision/ptext/psom) → 绝不可裸作路由信号。**B-1869** 登 catalog + audit.base (locator walk_fail fallback 报 success=True 测量隐患, axis-2 amplified; **post-fire candidate** 非 fire-blocker)。全链 → 笔记 §317-322。
> **⏭️ forward**: B1 cls **fire 6-mode 全完** (pprompt R32516 ✅); **`/diag pprompt b1` = discover 最后一个 (待跑)** → 完则 6-mode discover 齐 → **freeze step** (discover-then-freeze 协议): 合并 B1 dom+som+vision+ptext+psom+pprompt **6 condition** 规则提议 (WALK_FAIL[∪click变体] / MUTATION_MISSING / VISUAL_BLIND / DOM-URL-as-image / P33-severity 已收敛) → 落码 `diag_pattern_match.py` + bump `RULESET_VERSION` + `diag_autorun.sh` 全量重扫 → **解锁 cross-mode 定量** + B1 cls = 第2 完整 cell → k_cells=2 → §1 hero drop-one gate 出数 (现 INSUFFICIENT_DATA)。**cross-mode/cross-model 定量比较仍禁直至 freeze**。**P4 success-safe 收窄正解 = walk_fail 信号豁免** (非裸 element_id∈{0,1})。B-1869 留 post-fire 审。**digest 全在** `docs/analysis/vwa_classifieds/B1_{dom,som,vision,phantom_text,phantom_som}_classifieds_diag_digest.md`。
>
