...
> 🛑 **2026-05-25 fire 已停 + run-to-run noise 深挖 session**:cls fire(R2647 ptext)已 kill(queue_chain+runner+watchdog 全清),**SoM 编号决策 pending**。今天从 ptext repro 挖出 **H1/H3 hero 受 run-to-run noise 威胁**(element-ID churn = **P79 SoM/phantom 用 CDP nodeId 偏离标准 sequential SoM** + B0 MoE),GPT 三轮 cross-AI audit + SoM nodeId 发现。**复盘+深挖入口 = `docs/checkpoints/session_checkpoint_2026-05-25_runtorun_noise.md`**(self-contained:arc/定论/中途修正/待深挖/fire+文档状态)。⚠️ 今天文档改动**全未 commit**(含待修的笔记 §293 element-ID 错),reconcile 后再 commit。**fire 不要急着重启** —— 等 SoM 编号决策(改 sequential→重跑全 fire / 不改→续跑)+ noise mitigation 定。
...
> - **ptext repro 对比 done (§292; R2647 partial 67 task)**: archive R19776 (pre-B1860) ↔ current R2647 同 task 群 SR 28.4%↔25.4% (Δ-3.0pp 噪声内); **6 flip 全 model-nondeterm** (element_id flip 表象 harmless + 零起始污染, 两 run url_before 一致); **B-1860 对 P-text 无副作用确认**。工具 `scripts/analysis/compare_cross_run_same_condition.py` (通用 pre/post-fix 同 condition 跨 run 审计; ⚠️ 起始污染判定用 `url_before` 非 `obs_url`=action后)。**deferred**: R2647 完成后全量交集复算 (`--archive-run … --current-run …`)。
> - **H1 run-to-run sensitivity layer (§293, GPT cross-AI)** ⚠️ hero-threat: H1 drop-one 被 run-to-run noise 威胁 — prereg bootstrap 是 **task-level** 漏此方差 → anti-conservative (偏假阳); H1 strict effect 可能 1-2pp 与 floor 同量级。**mitigation = witnessed non-gating** (DOI 锁不加 gate)。**now**: 脚本 self-oracle symmetric discordance ✅; replicate-calibrated MC perturbation 设计待写; AMENDMENT_06 草稿。**fire-gated** (P0=不碰 running fire, 独立 namespace `results/repro_replicates/`): P1 B0 cls/red×P-SoM → P2 +competitors (**H_robust: P-SoM discordance ≤ {dom,som,vision}?** 赢=正面卖点) → P3 local deterministic 隔离 element_id (非 patch) → P4 P-SoM-unique challenge。**承诺**: floor≈effect → hero prose 主动降级。详 [[phase1_plan]] §D4 + [[paper_planning]] Risk 6。
