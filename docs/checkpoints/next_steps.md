...
> - **⚠️ Gate 3 disk (2026-05-22 R11315 health 暴露)**: results 实落 `/` (vda1 **93%, 37G free**), 非 cron 注释说的 /mnt/scratch (results 不是 symlink)。canary ~1.2GB **安全**; 但 full 36-cond ≈ 30-45GB > 37G → **可能填爆 root**。**Gate 3 go/no-go 第三因子** (除 wallclock+abort): redirect results→`/mnt/scratch` (**366G free, 24%**) 或 Fire 中 prune artifacts。注: 449G 用量 ≈ docker 衬底 volumes (root-owned, du-invisible), 非 P79 data (results 仅 878M)。
