...
**③ 写时状态 (2026-05-23 ~01:20 — 用①验证, 勿照搬)**: ✅ **Gate 2 DONE+PASS** — canary R11315 (B0×som×cls) **224/224 clean, retry=0 / evalerr=0 / abort=0** (结局①), SR 27.2%, latency ~110s/ep, cost $0.0757/ep, ~7.8h。最坏退化衬底零 abort = **强 full-Fire GO**(详 §273)。→ **Gate 3 active**(见 ④)。runner idle teardown 中 (CPU 0.1, watchdog 已退, Gate2→3 reboot 清)。canary=probe-only 数据不进 paper。Gate 1 (B-1836 eval zero-retry keyword fix, §269) + Gate 1.5 (pre-fire 3-AI /stress: codex 抓 fix **自身** P0-1 B-329 silent-score regression + gemini B-1837 differential-rescue, 全修 1273 test, §271) 已 land+push。prev abort R10016 superseded。dom **R9755 = dirty docker (跑前未 restart → 累积态比后续 fresh cell 脏) → 非 paper-grade, 进 36-cond fresh 重跑** (cell_b0_cls_dom `done` 待重跑后 cron 修正; 旧"不重跑"已作废)。**Amendment 02 witnessed** (2026-05-23, pre-data: git `e338cb4` + tag + OSF; H1-strict 6-mode gate 不动; 详 ④ + §274)。**B-1832/1835 image 修复已生产验证** (saga 关闭)。

**④ PENDING — 盯**:
> - ✅ **canary R11315 (Gate 2) DONE+PASS** (224/224, retry=0 evalerr=0 abort=0, SR 27.2%, ~7.8h = **结局① clean complete**, retry 安全网在位未动用 → 强 GO; §273)。**→ Gate 3 NOW** (分水岭已过, 顺序): ① **reboot A100** (修 NVML 580.159 + fresh docker + 清 R11315 idle teardown 进程) ② **results→/mnt/scratch** symlink ③ **每 condition restart docker** (防退化+消 latency confound, retry 留安全网) ④ 重算 **36-cond wallclock** (canary 数据点: B0 som ~7.8h/cond, 0 abort) + go/no-go ⑤ **36 条 fresh 重跑** ⑥ re-arm fire6 healthcheck cron ⑦ B-1837 measure eval-rescue rate ⑧ `/diag` per-condition。**framing**: 最坏退化衬底零 abort → fresh Gate 3 严格更易。**⚠️ canary = probe-only(非 paper data)**: 跑在 6-天退化 substrate; 若 Gate 3 = restart-per-condition(fresh, 推荐)则 latency 不可比 → **B0×som×cls fresh 重跑**进 36, canary 数据退 methods/appendix 当 Gate 2 证据。这 ~10-15h = **de-risk overhead, 非"36 里提前跑完 1 条"**(无论如何 36 都 fresh 重跑)。
> - **✅ Amendment 02 witnessed (pre-fire prereg requirement DONE, 2026-05-23)** — git `e338cb4` + tag `prereg-amendment-02-gate-ladder-20260523` + OSF, **pre-data 封存** (witness 时无 6-mode cell → 无 drop-one θ_i)。内容: H1-strict 6-mode gate **不动**(真赌注保留); 只 (a) 修 power-table estimand-label (+2.336pp = 4-mode ADD ≠ 6-mode gate) (b) 拓宽 post-R5 reporting (R5 仍 fires on H1-fail; route C'-S structure / C'-R router-only / F failure); anti-rescue guard 保留。**re-fire 现 gated on CODE only**(OSF/amend 已不 blocking): ① **docker-restart-per-condition**(卡 fire — 数据干净度)② stage3 ③ `cell_b0_cls_dom` 对齐(R9755 dirty→重跑)→ 齐了才 36-fresh fire。**post_r5_pivot 代码不卡 fire**(跨 producer, H1 失败后才需, 数据落地后写)。详 §274 + `docs/prereg_amendments/AMENDMENT_02_GATE_LADDER_20260523.md`。
