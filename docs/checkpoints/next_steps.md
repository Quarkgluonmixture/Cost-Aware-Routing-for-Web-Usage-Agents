...
**③ 写时状态 (2026-05-23 ~08:05 — 用①验证, 勿照搬)**: ✅ **Gate 3 fire LAUNCHED** — cls chain (B0 dom cls **R31194** 第一个) 跑中, 18 conditions sequential (B0/B1/B2 × 6 modes), 每 condition **B-1839 docker restart = fresh substrate**。本 session 全链 (§275): reboot A100 (NVML 580.159 fix → 60s 回来 driver 580.159.03 + nvidia-smi OK; fresh docker) → archive R9755+R11315 (probe → `_archive_pre_gate3_20260523`, A100+DGX, 都 non-paper-grade 进 36-fresh) → **B-1839 per-condition docker restart** + 3-AI /stress (gemini 独占 prereg-witness / codex 独占 sibling+timeout+app-layer / 全修 P1-1~3+P2-1~4, commit `06a303d`) → e2e 实测 (restart+token+完整 reset+seed pk≤84154 保留, 75s/cond) → homepage fix (reboot 遗漏, 见 §0⑤下 checklist) → launch。监控全自动: A100 fire6 (re-armed) + DGX paper_grade_check 6h + sync 15min。**~6-10 天** (B1/B2 latency cls 阶段实测; 实测 B0 cls ~8.3h/mode, 编排器估偏低 2× → 36-cond 重估 12-21 天)。canary/R9755 archive 不进 paper。Amendment 02 witnessed (`e338cb4`+tag+OSF, H1-strict 6-mode gate 不动; §274)。B-1839 OSF 不动 (衬底非 estimand, git tag `b1839-substrate-prefire` 凭证)。

**④ PENDING — 盯**:
> - **🔥 fire 跑中** (cls 18-cond, B0 dom cls **R31194** 起, sequential B0/B1/B2 × 6 modes; 每 cond **B-1839 docker restart** fresh): 监控全自动 → **新 session 入口 = 上方 ① `paper_grade_check` verdict** (别照搬本快照)。Gate 3 步骤已 done: reboot✓ / archive✓ / B-1839 docker-restart✓ + 3-AI /stress✓ (commit 06a303d) / wallclock✓ (12-21天) / re-arm fire6✓ / launch✓。**剩 (fire 进行中/完成后)**: ① 第一 cond 落地 → `/diag` per-condition (见下 bullet) ② **cls 18 完 → red 前**: scratch redirect (results→/mnt/scratch; cls 18≈13G 装得下 / 但全 36≈25G 需 scratch; sync `--delete-after` 风险 → 数据落 DGX 后做) + prune + **launch red 18** ③ B-1837 measure (见下) ④ Phase 1b: shopping reset 需含 docker restart (B-1839-fu, §4)。30min 健康 monitor `bqrj94mpl` (本 session)。
...
> - **Gate 框架 (user 2026-05-22) — 全 done**: Gate1✓(B-1836) Gate1.5✓(/stress) Gate2✓(canary R11315 clean) **Gate3✓ fire LAUNCHED (R31194 跑中)**。下一战略节点 = cls 18 完 → red 18 → Pass-1 36 齐 → `make analysis` + H1 gate。**fire6 healthcheck 已 re-armed** (line 53 "必须 re-arm" 已执行, superseded)。
