...
> - 过去 chronicle → [[实验笔记]] (latest §321, 2026-06-06 — §317-321 B1 cls cross-mode /diag discover: som finish-less/dom 真卡死/vision **router-crux REFINE**/ptext **axis-2 P4**; §319 lit-digest 8-paper 整合 §6)
...
> 🔥 **2026-06-06: B1 cls cross-mode /diag discover 推进 (4/6 mode) + B-1869 + 2 类 routing-rescuable 证据** ⭐ 最新。fire 健康自走: B0 cls 6 mode 全完 → B1 cls chain 自动推进 (dom/som/vision/ptext 落地, **psom 跑中 R26199**, pprompt pending; **fire 死活/进度跑 ①② 拿 live**, 别信本行秒级 stale)。本 session 4 condition /diag discover (**仅 discover 不落码**, ruleset 仍 `5-domsomvispsom-b1860coord`): §317 som (P31=finish-less artifact) · §318 dom (真卡死 + P31 跨模式 confound) · §320 vision (**router crux REFINE**: vision 只救 "AXTree 丢失的已渲染文字" 型 P5 [task40 dom30→vision4 flip 截图确认], 救不了导航/推理型) · §321 ptext (**axis-2 P4 root-ref 瞎猜** ptext41/vision0, observation+locator ground-truth 修正 sub-agent "[1]=RootWebArea" clean 故事 = §290 forensic 教训第4次)。**2 类干净 routing-rescuable failure** (paper router 硬通货): ① text-in-image rescue (§320 task40) ② attribute-verification deadlock (§321 task17 到达正确 item 但视觉盲确认不了属性)。**P31 finish-less 跨 4 mode confound 闭环** (som/dom/vision/ptext) → 绝不可裸作路由信号。**B-1869** 新登 catalog (locator walk_fail fallback 报 success=True 测量隐患, axis-2 amplified → §306 action_success 数 conservative; **post-fire candidate** 非 fire-blocker)。全链 → 笔记 §317-321。
> **⏭️ forward**: B1 cls 剩 **psom(跑中)+pprompt** → **6 mode 齐则 freeze step** (discover-then-freeze 协议): 合并 B1 dom+som+vision+ptext **4 condition** 规则提议 (WALK_FAIL / MUTATION_MISSING / VISUAL_BLIND / DOM-URL-as-image 已收敛) → 落码 `diag_pattern_match.py` + bump `RULESET_VERSION` + `diag_autorun.sh` 全量重扫 → **解锁 cross-mode 定量** + B1 cls = 第2 完整 cell → k_cells=2 → §1 hero drop-one gate 出数 (现 INSUFFICIENT_DATA)。**cross-mode/cross-model 定量比较仍禁直至 freeze**。B-1869 留 post-fire 审 (action_success gating vs locator_unresolved flag)。**digest 全在** `docs/analysis/vwa_classifieds/B1_{dom,som,vision,phantom_text}_classifieds_diag_digest.md`。
>
