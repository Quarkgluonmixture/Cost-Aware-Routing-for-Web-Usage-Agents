...
>
> **🔄 2026-05-25 UPDATE (B-1860 applied + restart from vision)**: B-1860 Qwen 0-1000 coord contract **APPLIED** (merge `d977006` = fix-coordinate-contract-b1860 @3ea598e into diag; amendment 05 witness `prereg-amendment-05-b1860-coord-contract-20260525` git tag push + OSF kv9sf upload DONE) + **B-1861** watchdog ntfy fail-safe (ntfy read-timeout=`socket.timeout` 漏 catch → watchdog 崩 → chain SIGTERM abort; 修 `_post_ntfy` catch-all)。cls chain **从 vision restart** = **手动 `queue_chain` 16 cells** (B0 dom R31194/som R9725 **保留**旧代码 [coord-invariant 不受 B-1860 影响], vision 起用新代码; ⚠️ **不能用 `queue_phase1 launch`** — 它 FORCE_NEW=1 会重跑全部含 dom/som)。**当前 B0 vision R24792 跑中** (新 contract 首个 vision)。R3671(旧 vision)/R1099(dom 半截)/R19776(ptext 180) 已 A100-archive。30min health monitor → ntfy `p79-claude`。WHY → §288。

**④ PENDING — forward-only (live 进度靠 ①, 不在此冻快照)**:
> - **每 condition 落地跑 `/diag`** (per-condition 命名, 勿用 run_id): 3-tier (Tier-1 0-token) → `docs/analysis/vwa_<site>/<model>_<mode>_<site>_diag_digest.md`, 拆失败结构喂 paper-grade 错因表。**已跑**: R31194 (B0 dom, §277) + R9725 (B0 som, §283 — SR 30.4%/scaffold=0/P14 presence-only/≥3 benchmark-FP[task96/42 复查完成=**B-21** string_match 货币 tokenize, P28 覆盖, 非 scoring bug])。**规则库已 self-evolve** (§284, ruleset 2-dom→**3-domsom**: P24-P30 落码 + P14 修正 succ-fire 15→0 + 全量重扫 dom+som)。**vision (R3671) /diag 完成** → 挖出 **B-1860 coordinate 根因** (Qwen 0-1000 vs P79 [0,1]/viewport 栈不匹配; 笔记 §285)。**✅ B-1860 contract APPLIED 2026-05-25** (merge `d977006` + amendment 05 witness git+OSF + B-1861 watchdog fix; §288)。R3671 archived; cls chain **从 vision restart** (手动 queue_chain 16 cells, R24792 vision 跑中新代码; B0 dom/som 保留旧代码 coord-invariant)。**下一个 diag**: **B0 vision (R24792 跑完 = 新 contract 首个 vision, 验证 parse_error 13.6%→正常 + true_oob no-op telemetry)**。
...
| ✅ **B-1860** (APPLIED 2026-05-25) | **Qwen 0-1000 坐标范式 vs P79 [0,1]/viewport 栈不匹配 → vision parse_error 13.6% + 48% ep cap-killed (R3671 /diag, 笔记 §285)**。Qwen3-VL 原生 0-1000 坐标, P79 栈全假设 [0,1]/viewport-pixel (prompt + `validate_action_detailed` + `vwa_wrapper.py:614/616` `/1280,720`) → ① 0-1000 标 normalized → validate reject (507 parse error=13.6%) ② 0-1000 标 pixel → wrapper 错归一化 (104 misclick 藏 SR)。铁证 y_max=972>720 viewport + `422=0.422×1000`。**vision SR 13.84% 非 clean (coordinate-scaffold 主导: 48% ep≥3pe/99% fail/cap 3-5 提前处刑), 不可跨 mode 比 SR / 不能只 disclosure**。Fix (**救 format 层不救 grounding 层**): contract=Qwen 0-1000 + wrapper `/1000` 映射 + validate[0,1000]+legacy[0,1] + prompt 改 0-1000 删 coordinate_type + cap 只在归一化后计数。**时机关键**: B0 vision 已污染(重跑); B1/B2 vision 本 fire 未跑 → **改 contract 应在 B1 vision 跑前** (否则一起污染又重跑)。**Per-model**: B0 铁证 / **B1 probe DONE** (DGX 角落 probe: 混用 [0,1]+0-1000; 底部项 (728,**920**) 920>720 排除 pixel = 0-1000 同 B0) / B2 Gemma 待 probe (B2 vision 前 A100)。**逐维度 contract: ≤1.1→[0,1] / >1.1→/1000。完整 fix @75b37bb** (branch `fix-coordinate-contract-b1860`: 主路径 3dd343a + /stress 7-项 extension = single-source `normalize_coordinate_pair` + 下游 diag/analyze + telemetry + B0 schema + select_option clamp; 40 test pass; B2 probe=Gemma normalized[0,1]→model-agnostic G-F1 defused)。R3671 已 **archived** (verdict 3→2)。**✅ codex verify 4 fix-impl bug 已补 @3ea598e** (fix 分支): V-F1 true_oob fail-closed no-op (4 wrapper site, `dispatch_path=coord_true_oob_noop`) / V-F2 下游 negative coord 显式计 OOB / V-F3 dead_zone aggregate counter / V-F4a annotate import normalizer (watchdog fire-time 自动标注); **V-F4b glm_batch_digest 撤=半退役** (user catch: B-743 standalone, 不在 cron, diagnosis 被 /diag 取代, 不在 fire 路径) + 5 test (targeted pytest **94 passed 0 回归**)。**✅ APPLIED 2026-05-25** (实际触发非"B1 vision 前窗口" — 是 **ptext abort via B-1861** 趁 chain 死窗口 apply): ①补 4 bug+test ✓ @3ea598e ②merge fix→diag **`d977006`** ③witness tag `prereg-amendment-05-b1860-coord-contract-20260525` + OSF kv9sf (amendment 05, 披露 instruction-strictness relaxed + HARKing) ④A100 pull diag ⑤**手动 `queue_chain` 16 cells 从 vision restart** (⚠️ 教训: **NOT `queue_phase1 launch`** — 它 FORCE_NEW=1 重跑全部含 dom/som; 且 cells 必 quote 否则 55-token bug; B0 dom R31194/som R9725 **保留** coord-invariant, vision 起新代码)。**当前 B0 vision R24792 跑中**。R3671/R1099(dom 半截)/R19776(ptext 180) A100-archived。+ **B-1861** watchdog ntfy fail-safe 同期修 (ntfy read-timeout=socket.timeout 漏 catch → watchdog 崩 → chain abort)。**post-fire**: dom/som digest version-stale 重扫 (3-domsom → 3-domsom-b1860coord)。 | DONE (apply ~1.5h 2026-05-25) | **✅ APPLIED** — restart from vision (R24792 跑中, 新 contract); dom/som 保留; 待 B0 vision /diag 验证 parse_error 13.6%→正常 |
| 🟡 **Repro-replicate** | **Run-to-run SR 方差: 机制确定 + 量级 OPEN (深挖闭环, 笔记 §282 / B-1858)**。**机制 (实证)**: VWA element-ID 非确定 (`processors.py:532` CDP/树序号) → 同页面 obs byte-diff (**仅 `[id=N]` 变**, content-diff 证结构稳) → temp=0 模型对 ID token 敏感 → action churn → 轨迹分叉; 截图=视觉锚 (SOM step-0 action 90%>DOM 73%)。**量级 OPEN — 无干净 replicate** (两 pair 都 confound): DOM (R31194 vs R9755)=regime (fresh vs pre-B-1839-stale), 0.4pp 对称 = regime≈0 proxy **非纯 replicate**; SOM (R9725 vs R2815)=R2815 是 **B-1848 wedge run**, +7pp **非对称** (19/6 p=0.015)。判别 (telemetry + **full 25-flip 轨迹扫**: 全 clean agent failure, t151 minSoM=2=agent 点图非 substrate, errors=`no_progress` symmetric) **排除 A/A' → B=真 agent run-to-run 方差 confirmed across all 19 + directional** (n=1 可能 fluke / ① model drift)。**codex gpt-5.5 cross-AI re-derive 确认+3修正** (→ `codex_outputs/repro_directionality_causes_2026-05-24.md`): [id=N]=AX `node.nodeId` (非 backendDOMNodeId/cursor); "仅 ID 变" **主导非普适** (step-0 137/37/11, flips 20/4/1); **① model-serving 未排除** (4/25 flip step-0 byte-identical 却 diverge=provider nondeterminism)。**完整 3 源**: ②ID-churn 主导(20/25)+①provider-nondet 真实(4/25)+③site-state 少量(1/25 task145); 方向=chance+post-hoc(template 15/5/1 p≈0.041)。**n=1 因果至此 diminishing returns; 剩=clean replicate (数据非分析)**。**撤回**早前 "0.4pp clean floor / 对称 / gate robust"。**gate 风险 OPEN**: som run-to-run 可能几 pp → 接近 phantom 1.7-3.3pp → 可能威胁 gate; dom<1pp 不背书 som。**Forward**: (a) **🔴 clean 3rd som replicate (两健康 B0 som) = gate 前置** (估真 run-to-run std + 是否 directional/复现); (b) §4 disclosure 用 replicate 数非 R2815; (c) post-fire replay 切 ①/② + positional-ID fix (B-1858 根除 ②); (d) phantom 落地测 step-0 分叉。 | (a) **gate-blocking** post-fire/idle · (b)-(d) post-fire | **gate 前置 (a)** / post-fire |
