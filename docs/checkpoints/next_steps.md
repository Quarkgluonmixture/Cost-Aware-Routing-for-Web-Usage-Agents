...
> [!todo] Top forward actions (priority order, **2026-05-19 evening update** — Fire-3 DEAD + Phase 0 /stress operational hardening landed; cold re-fire pending push)
> 1. **🔥 Fire-3 COLD RE-FIRE pending push** ⭐⭐⭐ — Fire-3 cls B0 DOM aborted 2026-05-19 00:53 BST (eval `page.goto` 30s timeout × 3 → B-486 quarantine; task 75 specifically) + red B0 DOM 4h wallclock kill 04:57 BST (cascade caught by P0-2-B* SMOKING GUN — master `queue_phase1_paper_grade.sh` `kill -0` did not read exit code → cls abort silently launched red). **F1 async_envs.py asyncio leak RCA RETRACTED** — P79 never imports AsyncScriptBrowserEnv; Probe A (B1 task 75 isolated) + Probe B (B1 tasks 70-75 sequential) both reproduce zero eval timeout despite 4× 30-step episodes accumulated. Phase 0 /stress 3-AI audit (Mode A Claude + Mode B codex + Mode C gemini) produced **18 unified findings** (8 P0 + 9 P1 + 1 P2); Phase 1 operational hardening (commit `04e1d9b`, 11 fixes) + Phase 2 additive telemetry (commit `8d2a327`, 24 new schema fields + 1 sync invariant test) landed. **Next**: /stress on Phase 1+2+3 diff → push → cold re-fire cls B0 DOM from task 0 → sequential cls→red chain via fixed pipeline. 详 [[实验笔记]] §237.
