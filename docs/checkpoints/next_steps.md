...
> 📅 **2026-06-10 advisor 两份书面 deliverable + deadline 自查 (standing, 每 session 必查)**: **D1 one-pager → advisor 06-13** (`deliverables/advisor_onepager_2026-06.md` 已起草待 review) · **D5 lit-review 章 → advisor 06-27** · D4 Pass-1 全 36 cond 06-26 · D6 full analysis + drop-one verdict 07-01 · D7 router H10 verdict 07-08 · D9 thesis draft v1 08-10 · D11 submission early-Sep (**TBC 待确认**)。全表 = one-pager §4 (canonical); live = `make status V='tasks#NOW'` (eta 字段)。**每 session handoff 对照自查 on-track, 偏差主动报 + 提议 re-plan** (advisor 显式要求 "always check that you are on track")。详 `issue_advisor_sync_2026-06-10` + 笔记 §331。
