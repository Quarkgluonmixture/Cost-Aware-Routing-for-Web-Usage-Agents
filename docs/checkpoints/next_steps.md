...
> - **仍 pending**: **N=5 same-payload replay** (no GPU-hour, `probe_proxy_full_stack.py` adapt `tool_choice="auto"→"required"`, 20 task × N=5 back-to-back, 二分 within-minute vs 24h-drift 定 codex #1 vs #2) · **B-1867 audit artifact patch** (Phase 1b 加 request_id/instance_id/SHA/response-headers/logprob-margins 持久化 → 未来 fire 真有 forensic data) · replicate-calibrated MC perturbation (post-fire, AMENDMENT_06 non-gating, 现 reframe 不针对 MoE 而是 "remote-serving floor") · 承诺 floor≈effect → hero prose 降级 · Tier B v3 per-position patching (**scope 紧, §5 advisor 暂搁仍生效**)
>
