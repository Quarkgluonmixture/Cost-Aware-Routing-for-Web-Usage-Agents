...
> [!todo] Top forward actions (priority order, **2026-05-19 evening update — Phase 0 R1+R2 closed + origin/master pushed; Fire-4 cold re-fire trigger pending**)
> 1. **🔥 Fire-4 cold re-fire trigger pending** ⭐⭐⭐ — Phase 0 /stress 2-round closure complete: R1 (substrate audit) 18 findings → 11 fixes (`04e1d9b`) + 24 schema fields (`8d2a327`) + docs (`a3e611a`); R2 (Mode B+C regression on R1 diff) 9 push-blockers → 9 fixes (`492acb0`) + docs (this commit). All 5 commits pushed origin/master at `492acb0` 2026-05-19 evening BST. **F1 async_envs.py RCA retracted**. Pre-fire checklist on A100: (a) `git pull` (5 commits), (b) `rm` 3 stale .locks (B1 probe residue), (c) trigger `RESET_BEFORE=1 bash scripts/queues/queue_phase1_paper_grade.sh launch` — sequential cls→red, baseline-aware wallclock (B0 8h / B1+B2 4h), P0-2-B* sentinel-wait halts cascade on any failure. Expected wallclock Pass-1 ~13-18 days. 详 [[实验笔记]] §237 (R1 RCA + Phase 1+2+3) + §238 (R2 9 push-blockers + cross-AI ROI lesson + Fire-4 pre-fire checklist).
