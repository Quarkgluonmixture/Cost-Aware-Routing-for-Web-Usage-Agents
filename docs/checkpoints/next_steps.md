...
> ## 🟡 2026-09-15 晚 · Showcase（09-16）片子改了一轮，user 仍不太满意，开新 session 继续 · 先看这里
>
> **当天手册（手机可开）**：https://claude.ai/artifact/HHEK4g5x1G9irBhPvZU67x（Version 9）← 源文件 `deliverables/showcase/day-of.html`。它只是汇总；事实变了先改源文件：`ROADMAP.md`（D15–D23 · Phase 5/6）· `talk/RUNBOOK.md` · `demo/README.md` → Live · `talk/rehearsal-script.md` · `SHOWCASE_PREP.md` §2 走读 / §4 问答 / §5 数字口径 / §6 禁语
> chronicle → **笔记 §511–§526**（本 session：§518–§526）· commits `90544a5` … `d52aa12`，**全部未 push**
>
> ### 现在的片子（正文 11 页，页面 id）
> opening · **agents**（同一张 Wikipedia 注册页的两份真实抓取，放进 Claude Code / Codex 窗口框，D22–D23）· **prize**（perfect router 四张数字卡）· demo · question（3 个路标）· behaviour · failure · learned（perfect router 措辞 + 简化散点图，D21）· why（真实 6 点 + 训练分界线，D20）· not-yet · close · reference
>
> ### user 收尾时说「还是不太满意」，没说具体哪里 —— 新 session 先问清
> 本 session 来回最多的是开场：写死的动画（学长：hard code、和 demo 重复，**未提交**）→ 真实抓取两图 → 放进工具窗口框。以下只是记下的可能方向，**未经 user 确认**：开场信息仍偏密 · agents 与 prize 要不要合成一页 · 台本偏长。
>
> ### 先跑命令确认现状（别照抄旧数字）
> - 片子：`.venv/bin/python3 deliverables/showcase/talk/check_talk.py`（09-15 收尾时 PASS；台本约 932 词，**偏长，未出声掐表**）
> - 真实抓取素材：`.venv/bin/python3 deliverables/showcase/talk/real_capture.py render`（素材已入库；`capture` 要联网，本机 MCP 需 `--no-sandbox`）
...
> - 离线包 `tmp/showcase_talk_bundle.zip` 不入库；片子一改就 `cd deliverables && zip -u ../tmp/showcase_talk_bundle.zip showcase/talk/<改过的文件>`，再改 `day-of.html` 里的版本时间
>
> ### 还没做的
> 1. 问清 user 哪里不满意再改；每改一次走完：check_talk → export_pdf → 看 PDF 对应页 → 更新离线包 → 手册版本号 → 提交 → 重新发布手册
> 2. 台本出声掐表（约 932 词；超过 9:30 就按 `rehearsal-script.md` 顶部的裁剪顺序砍）
...
> - agents 两图是 Playwright MCP 对真实网页的抓取，**不是 Claude Code 或 Codex 的运行记录**；Codex computer use 是截图加无障碍文本混用，不说「Codex 只看截图」；「GPT-6 Astra 只看截图」记为 CLAIM_UNVERIFIED（§524.1）
...
>
> **② ~~横排合法性~~ 已消解（08-27 晚，笔记 §485）**：全稿横排 **0 处**。
> user 指出会议稿里 `fig_overview` 本来就是竖排发表的 —— 查证属实（NeurIPS 单栏
> `0.90\textwidth = 12.57cm`，实测图内最小实词 0.96pt）。毕设版心 13.0cm 放 `\textwidth`
> 比原发表尺寸还宽 3%，所以竖排是**原样复现**而非妥协。`PRINT_W_IN` 那条 ≥8pt 规则的适用域
> 已限定为「本仓库自己重画的图」。
>
...
>
> chronicle → **笔记 §473**（8 小节）· 台账 **+21**（2438 → 2459）· 卡 → [[task_vlm4rwd_workshop]]
>
> REALM 稿换 NeurIPS 模板投 [vlm4rwd.github.io](https://vlm4rwd.github.io/) 非归档 track,
> **已提交**（deadline 08-31 05:00, 以 OpenReview 表单为准）。notif **2026-09-29**, 此前无动作。
> 非归档 ⇒ 不占 NAACL ARR 投稿权, 与 REALM #192 不冲突。产物 `deliverables/vlm4rwd/`（已 push）。
>
> **下面那条 reframe chain 才是主线, 本条不影响它。A100 一个字没动。**
>
> ### 四条会再咬人的教训 —— 全是同一形状「检查全绿而东西是坏的」
> | | |
> |---|---|
> | **`wrapfigure` 静默裁 caption**（§473.1） | 超出预留行数的部分直接丢弃、不报 warning。丢掉的三段（含论文核心主张）在整个 PDF 里零命中, 而 `latexmk` exit 0 / 0 error / 0 undefined / **页数达标** 全过 —— 页数达标恰恰是字被吃掉的结果。**paper 目录此后禁用 wrapfigure** |
> | **`pdftotext \| grep` 查不出匿名性泄漏**（§473.8） | `\includegraphics` 嵌入 PDF 时 pdfTeX 把**源文件绝对路径**写进 XObject 字典的 `/PTEX.FileName` —— 不是页面文本, 只有 `strings` 看得见。**四渠道审计**: strings / pdfinfo 元数据 / XMP / 附件。清理用 `gs -sDEVICE=pdfwrite`（`mutool clean -ggg` 无效, PTEX 是字典 key） |
> | **投稿有两个表面, 只审了一个**（§473.7） | OpenReview 的 abstract / keywords / TL;DR 手工粘贴, **不随 PDF 重编译更新**, 而 keywords 决定 reviewer 匹配 ⇒ 比 prose 更早生效。三家 AI 审的全是 PDF |
> | **NeurIPS 系 workshop 一律附 checklist**（§473.8） | 即使 CFP 没点名。官方 `checklist.tex` 自己写着「papers not including the checklist will be **desk rejected**」, 且不计页数 ⇒ 加了几乎无成本, 不加是赌 |
>
> ### REALM #192 的状态（camera-ready **09-14** 是唯一窗口）
> - ✅ **Overleaf 源图已清理并 push**（`6480bc0`）⇒ camera-ready 重新编译即干净
> - 🔴 **在审的那份提交件仍带 `/PTEX.FileName` 泄漏** —— 审稿阶段换不了, 只能等 camera-ready
> - 🟡 **三处 prose bug 未修**: `tab02` 引 `tab:t04` 论证 non-separability 而 `tab05` 明写「This is not a separability test」且阈值是 87.5% 非 83%（**`tab05` 一直在纠正 `tab02`, 但 `tab02` 从没改**）· `paper-B` 源项目残留 · Multiplicity 段落被归进 Limitations。VLM4RWD 副本三处都已修, 可直接照搬
...
>
> ✅ **尾链已自动接上（08-06 11:31 实测确认）**：`[shop-tail 11:30:02] dom 主臂完成 (complete eps=435)
> → launching` → 8 道 gate 全过 → `[11:31:32] LAUNCHED chain pid=109050`，`.fired` 已写
> （B-1964 三态机生效：只在 chain 确认起来后才写终态）。整条设计路径**首次完整走通**，无人工介入。
>
> ⚠️ **排查时踩的一个坑**：11:20 那轮 cron 与 `condition_summary` 落盘撞在同一分钟，读到的还不是
> `complete`，于是走了「安静等待」分支（该分支有意不打日志）。据此推断「脚本零输出 = 有 bug」
> 并连查四轮（HELD 去重 / cron 停摆 / 僵尸进程 / 早退分支），全被证伪——实际只是**查早了两分钟**。
> **判据：follow 类脚本的静默是设计而非故障；断定它卡住之前，先等过一个完整 tick。**
> 拉实时，别照抄这里的数字：
>
> ```bash
...
> Overleaf 已推（`7e9f7bb`，19 页 / 0 undefined citation）。面板 = `deliverables/duizhang_board_2026-08-04.html`（40 格 / 13 条展开，gitignored）。
>
> ### 一句话
> user 问「证据层是不是彻底完结」→ 跑 `/stress` milestone（Claude + codex，双方 stats methodologist）。
> **12 条发现零重合，但形状一致：取了极值/做了挑选却不校正。** 这不是十二个 bug，是一个系统性缺陷的十二个实例。
>
> ### 🔴 会改变你怎么引数字的四条
>
> | 结论 | 影响 |
> |---|---|
> | **「24/24 胜随机」已删** | 那是 max over 8–10 signals 减常数。改引**全体 222 点 64.4% 为正 / 中位 +0.327pp**；`wa_red_B1` 三档中位全负而 max 全正 |
> | **合取校正后 off-segment 6→4，Holm 1→2** | BH 原打在 128 条腿上而非 64 个「两条都成立」的假设 —— 那是 filter 不是 test |
> | **`WA·B1` 唯一的「超出重跑地带」判定翻转成 inside** | 原带宽是 5 模式池化在同一批 10 任务上；未池化真实范围 0–10pp。**Table 18 现在两个有地板的格子都 inside，claim 1 零反例** |
> | **「点击/打字失败率」不是纯条件率** | 无分母 episode 被记 0（满分）再平均；**25–35% 的 episode 从没打过字**。新增 `*_complete_case` 列，任何「谁失败最多」要从那读 |
>
> ### 🟡 下一轮
> 1. **P1-5 / P1-6 未做**（reference-image 双重挑选 · 难度趋势每层换比较对象）—— codex 实测「1/6」是 1/6~4/6 区间的下端
> 2. **family 声明里「哪些表算确证」标着 TBD** —— 是定 frame 时的决定
> 3. 两个 commit 未推 GitHub
>
> ### ⚠️ 四轮下来同一个教训的四个版本
> §427.7 只报位移不报收敛 · §427.8 只测「访问过」不测「是否订阅」 · §427.9 只看覆盖矩阵不看产物自己的限定语 · §428 只取极值不报未选中的分布。
> **报告一个经过选择的量时，未被选中的那部分才是它的参照系。**
