...
> - **每 condition 落地跑 `/diag`** (per-condition 命名, 勿用 run_id): 3-tier (Tier-1 0-token) → `docs/analysis/vwa_<site>/<model>_<mode>_<site>_diag_digest.md`, 拆失败结构喂 paper-grade 错因表。**已跑**: R31194 (B0 dom, §277) + R9725 (B0 som, §283 — SR 30.4%/scaffold=0/P14 presence-only/≥3 benchmark-FP[task96 url-EXACT 双满足却 0 分=评测器最可疑])。**规则库已 self-evolve** (§284, ruleset 2-dom→**3-domsom**: P24-P30 落码 + P14 修正 succ-fire 15→0 + 全量重扫 dom+som)。**下一个**: vision (R3671 跑完即可 diag, 用 3-domsom)。
