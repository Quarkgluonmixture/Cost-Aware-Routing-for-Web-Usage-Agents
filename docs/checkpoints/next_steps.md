...
> - **✅ committed + pushed** `origin/fix/b1878-reddit-reference-image` (4 fire-path 文件 + catalog + 笔记 §349, user 显式确认)。**re-launch 同 session 执行**: archive R28130 partial (58 ep, 无 condition_summary) → `RESUME_MISSING=1 MAX_CONDITION_HOURS=0 MAX_CLS_WAIT_HOURS=0 launch red` (必 `red` 非裸 launch); live 进度跑 `make ntfy` + ① verdict (别信本行硬编码)。
