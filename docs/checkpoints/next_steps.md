...
> - **每 condition 落地跑 `/diag`** (per-condition 命名, 勿用 run_id): 3-tier (Tier-1 0-token) → `docs/analysis/vwa_<site>/<model>_<mode>_<site>_diag_digest.md`, 拆失败结构喂 paper-grade 错因表。**已跑**: R31194 (B0 dom, §277) + R9725 (B0 som, §283 — SR 30.4%/scaffold=0/P14 presence-only/≥3 benchmark-FP[task96/42 复查完成=**B-21** string_match 货币 tokenize, P28 覆盖, 非 scoring bug])。**规则库已 self-evolve** (§284, ruleset 2-dom→**3-domsom**: P24-P30 落码 + P14 修正 succ-fire 15→0 + 全量重扫 dom+som)。**vision (R3671) /diag 完成** → 挖出 **🔴 B-1860 coordinate 根因** (Qwen 0-1000 vs P79 [0,1]/viewport 栈不匹配; vision SR 13.84% **非 clean** — 48% ep 被 coordinate-scaffold cap-killed, 不可跨 mode 比; 笔记 §285)。**vision 需修 contract + 重跑** (见 §4 B-1860, gate-blocking, **B1 vision 跑前决策窗口**)。**下一个 diag**: B0 P-text (R19776 跑完)。
