...
| 🟡 **Repro-replicate** | **意外全复现 replicate → paper §4 reproducibility disclosure** (B-1848 hang 副产品)。R31194 (Gate-3 fresh) vs R9755 (Fire-6 stale 衬底, `_archive_pre_gate3_20260523/`), 两个**完整** B0 dom cls 224-run: 聚合 SR 复现 **0.4pp** (15.2 vs 14.7), per-task agreement **88.8%** (199/224), discordant **对称** (cur✓old✗=13 / old✓cur✗=12, McNemar 精确 p≈1.0) → 单次 SR 的 per-task 随机 churn (~11%, web-env timing/obs/站点微状态非确定性, NOT eval-drift) 在聚合层**对称抵消**; 不同衬底 SR 不变 (衬底差异 = latency confound 非 SR)。**Forward**: (a) 写进 paper §4 = "每 condition 只跑一次, SR 稳吗" reviewer 攻击的实证 defense; (b) R9725 som 长到大 N (~185) 后 redo matched vs R2815 (archived), 确认 som flip 也回归对称 (N=25 的 3-0 NEW-favorable = 小样本噪声, 非 fresh-衬底 SR 优势 — dom N=224 已证对称)。 | (a) 30min 写 · (b) R9725~185 时算 | paper-write phase / som re-check 当 R9725 ~185ep |
