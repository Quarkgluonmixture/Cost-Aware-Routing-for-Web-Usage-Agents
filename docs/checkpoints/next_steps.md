...
> | **08-03** | **周一** | 和学长对账 → 骨架 |
> | 08-04 → 08-05 | 二～三 | 骨架落地 + 两篇稿 splice |
...
> ### ⭐ 因此周末的最优解：先写 Paper B
>
> `task_realm_paper_b_router_negative` 自己写明「**⭐ 不阻塞于 k=6**：核心证据是**结构性事实**，
> B2_reddit 进来只多一行，论点一字不改 → **现在就能整篇写完**」。它同样**不依赖对账结论** ——
> 阴性结果 + 标签供给诊断是已经落地的数字（`router_label_supply_diagnosis` 07-28 重建，k=6 实测）。
>
> 所以 08-01/02 这两天不是空等：**Paper B 可以整篇写完**，把 08-03 之后的 2 天全部留给 Paper A。
> 反过来若周末不写，两篇稿会在 2 天里挤在一起。
>
> Paper A 依赖对账（骨架未定时写的 prose 会在结论裁定后大概率推翻重写 —— 07-28 起走零预设重建
> 的原因），所以它**只能**等 08-03。
>
