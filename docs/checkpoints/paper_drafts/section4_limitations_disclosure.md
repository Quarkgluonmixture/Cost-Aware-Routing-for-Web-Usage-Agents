...
`p79-patches`, HEAD `2c15d66d120f8498633ae65057aa50a34b3e93e0`. Upstream base is
...
| `f883a11` | fire-event lock: untrack generated test_shopping.json | `git rm --cached config_files/vwa/test_shopping.json` (URL-substituted generated file); **zero behavioural impact** | `config_files/vwa/test_shopping.json` (untracked) | §4.X.11 row, §4.X.12 |
| `ac33d2f` | fire-event lock: `.gitignore` typo + manifest | `.gitignore` L144 slash-typo fix (`vwatest_shopping.json` → `vwa/test_shopping.json`) + add `generation_manifest.json` entry; restores 3-site symmetry; **zero behavioural impact** | `.gitignore`, `config_files/generation_manifest.json` | §4.X.11 row, §4.X.12 |
| `2c15d66` | C1b screenshot-timeout non-fatal recovery | On a heavy-DOM page where `Page.screenshot` exceeds 30s, `AsyncScriptBrowserEnv.astep` returns a blank ndarray + `info['screenshot_timeout_recovered']=True` instead of raising (`content()` captured first). Mode-AGNOSTIC at the submodule; the dom-only vs som/vision fatality decision lives in P79 `VWAWrapper._gate_screenshot_timeout`. **Behavioural impact**: dom-mode (artifact-only screenshot) episodes no longer quarantine on the Fire-4 task-75 `agent_observation Page.screenshot` timeout class; som/vision/phantom_* (decision-input) re-raise as before; screenshot still attempted every step. **Latency treatment (B-1780, /stress GRL audit 2026-05-20, Q3=A — supersedes the earlier "comparability preserved" claim)**: the recovered ~30s timeout IS recorded in **raw** latency (`total_latency_ms`, operational truth) but EXCLUDED from **canonical** latency (`total_latency_canonical_ms = minus_retry − busy_wait − recovered_screenshot_timeout`), because the dom artifact screenshot is not a decision input so its infra wait must not inflate dom efficiency vs SoM/Vision. The per-cell `screenshot_timeout_recovered_episode_rate` (mirrors B-486 `quarantine_rate`) is reported as a sensitivity column bounding the adjustment magnitude. Earlier wording asserted comparability without the adjustment wired (the B-1773 fields were write-only); B-1780 wires the exclusion + rate. | `browser_env/async_envs.py` (+ P79 `p79/envs/vwa_wrapper.py`, `p79/experiment/runner/main.py`, `metrics.py`, `aggregate_cross_site.py`) | §4.X.11 row, §3.5.1, prereg §7 (SBOM re-lock) |
...
git rev-parse HEAD                                 # must match 2c15d66d120f8498633ae65057aa50a34b3e93e0
...
# must match 2696d0a61e2f70536f247ebb225f51c262b657d8b8b7b407f8581b75757a8bae
```
...
Phase 1a fire writes `trajectory_events.jsonl` (Option K, B-313~B-384 + B-742 auth_refresh_no_clear hook + B-1868 session-loss preserve events) per condition_dir, recording all auto-clean / auto-refresh / reset / session-loss perturbation events. Paper §4 uses this log to emit per-episode covariates (`is_after_reset` / `had_auth_clear` / `had_finalize_race_clear` / `cleared_in_session_wave` / `session_lost_preserved` / `session_lost_preserved_wave_size`) for GLMM fixed-effect adjustment. The `session_lost_preserved` covariate is **paper-grade-mode-specific** and **emitted by disjoint watchdog paths** from the dev-mode `had_auth_clear` / `cleared_in_session_wave` covariates under intended runs; the aggregator computes the two channels independently (no enforced mutual exclusion), so a hypothetical simultaneous-True case would indicate mixed / corrupt artifact history (e.g. dev→paper-grade run-mode swap mid-condition) rather than a normal state. Under `P79_PAPER_GRADE=1` the watchdog preserves contaminated episodes rather than deleting + retrying (B-1868 / PROTOCOL_NOTE_01 — see §3.5.4), so the covariate marks the preserve event rather than a clear event. Aggregator OR-fuses an event-log read with a canonical-summary `infra_covariates` field read so a single-channel gap (logger failure, watchdog crash, race window) does not silently zero the covariate. Event writes are **best-effort post-hoc enrichment** rather than 2-phase committed transactional audit trail: in `scripts/maintenance/experiment_watchdog.py` the event write happens AFTER the destructive op (`unlink` + `rmtree`; post-B-743 digest retire 2026-05-17, no longer includes `_purge_digest_records` step), creating a race window in which SIGKILL/OOM/`restart_watchdog.sh kill -9` drops the event while the filesystem mutation persists. The B-1868 dual-path (event + summary `infra_covariates`) closes this race for paper-grade preserve events specifically; legacy dev-mode events retain the single-path event-log semantics.
...
### Runner-intervention attribution and B-486 quarantine event rate (Phase 0 /stress 2026-05-19)

**Context**: Phase 0 /stress 3-AI cross-AI audit (2026-05-19) identified two operational hardening gaps that were closed in commits `04e1d9b` (operational hardening, 11 P0+P1 fixes) + `8d2a327` (additive schema + telemetry, 24 new fields). Both are forward-disclosure additions per preregistration Appendix E.

**About:blank recovery and `page_changed` attribution (P0-1-ABC* 3-AI overlap OOB)**: when the agent's emitted action lands on `about:blank` (Playwright transport edge case during page-load timeouts), the runner intervenes via `self.environment.navigate_to(task.raw_task["start_url"])` to restore the episode to a valid evaluation substrate (`runner/main.py:2450-2473`). Pre-2026-05-19 hardening, this recovery was silently appended to the step record's `page_change_reasons` list, making `page_changed=True` for the intervention step. Downstream consumers of `page_changed` (paper §3 step semantics, `no_progress` denominator, cycle/retry detection) treated the runner-initiated navigation as agent progress.

Post-hardening (`runner/main.py:2483-2491`): `page_changed` reflects agent action only. Runner intervention is recorded separately via step-record fields `intervention_type` (= `"about_blank_recovery"` when fired), `counted_as_agent_action=False`, `intervention_from_url` (page URL before intervention), and `intervention_recovery_url` (target after `navigate_to`). Episode-level rollup `runner_intervention_count` + `about_blank_recovery_count` (defaults 0). Aggregator (`scripts/analysis/about_blank_frequency.py` P1-11-B* extension) emits attribution columns: `runner_intervention_step_rate`, `agent_progress_legacy_step` (pre-fix archived rows where attribution was silently agent-credited), `agent_progress_url_only_step` (raw URL signal, no metadata), `page_changed_minus_intervention_step` (agent-only page-change rate, intervention subtracted), `agent_visible_changed_step`.

Mixed pre/post-Phase 2 archive aggregation is handled via **column-level attribution split** at `scripts/analysis/about_blank_frequency.py` (per the `_classify_about_blank_attribution()` helper landed in commit `8d2a327`). The aggregator emits distinct columns per condition row — `agent_progress_legacy_step` (pre-fix archived cohort), `runner_intervention_step` (post-fix attributed Phase 2 cohort), `agent_progress_url_only_step` (raw URL signal, no metadata), `agent_progress_explicit_step` (Phase 2 with `counted_as_agent_action=True`) — rather than producing separate cohort rows. **This differs from B-554 evaluator_authority_mode cohort isolation, which separates rows by archive epoch**; the about:blank columns are within-row sub-attributions on the same condition substrate. Pre-fix Fire-3 partial archive (75 cls episodes, aborted at task 75) is NOT used for canonical claims; Phase 1a re-fire post-hardening is the primary substrate.

**B-486 quarantine event rate (P0-8-C* Gemini OOB)**: when an episode's evaluator phase exhausts 3 retries on Playwright navigation timeout / dep exception / OpenAI 503, the runner raises `EvaluatorUnavailableError` (`environment.py:285-422` per B-486 paper-grade fail-loud contract). For the evaluator-fail trigger path, the runner **re-raises before writing `episode_summary_v2.json`** (`runner/main.py:1505`) — therefore the affected episode has NO summary on disk with `needs_reevaluation=True`. Quarantine evidence is preserved via **(a) chain-level done-sentinel rc** (P0-2-B* fix: master `queue_phase1_paper_grade.sh:738-768` reads sentinel rc, halts cls→red cascade on rc≠0) and **(b) runner log artifacts** (traceback + `EvaluatorUnavailableError` message + 3-retry log lines in `logs/<run_id>_runner.log`). The P1-10-B watchdog `needs_reevaluation` field check (`experiment_watchdog.py:1789-1815`) protects the **non-evaluator** quarantine class (runner crash mid-episode where summary IS written with the flag) — forward-compat for future quarantine variants AND defense-in-depth for non-evaluator failure modes.

Cross-baseline asymmetry is expected: B0 (AWS proxy, ~24s/step latency) has higher Playwright timeout exposure than B1/B2 (local 4B inference, ~2-5s/step) at high task count substrates. Phase 1a re-fire will measure per-cell `quarantine_rate = quarantined_episodes / scored_task_count` as an Appendix Sensitivity column with pre-registered invalidation thresholds (preregistration Appendix E.1: > 5% per cell flagged + reported; > 20% per cell excluded from primary FE gate). Repeatedly-quarantined tasks (3 manual re-fires all fail) are mapped to `success=False, score=0.0` to preserve `scored_task_count` denominator (NOT silently dropped). The Fire-3 cls B0 DOM 2026-05-19 00:53 quarantine on task 75 was an empirical anchor for this attack vector; Probe A (B1 task 75 isolated) + Probe B (B1 tasks 70-75 prefix) both reproduced zero eval timeout, indicating the gap is B0/substrate-scale-specific (75-task dwell time × ~24s/step ≈ 15h vs B1 Probe B 5-task × ~2s/step ≈ 10 min — 4 OOM dwell-time differential not directly testable without cost-prohibitive B0 prefix replay).

**Reviewer attack defused**: (a) "page_changed conflates agent + framework" — page_changed now agent-only; intervention separately attributed via Phase 2 step-record fields. (b) "B0 quarantine triggers more frequently than B1/B2, silently inflating SR" — per-cell `quarantine_rate` as Appendix column with pre-registered 5% / 20% invalidation thresholds (preregistration Appendix E.1); chain-level + log-level forensics preserve quarantine evidence (no silent re-roll); 3-strike terminal mapping prevents denominator shrink. (c) "cross-baseline asymmetric scaffold effects undisclosed" — explicitly disclosed here as Limitations. (d) "decision rules unchanged claim is misleading because page_changed semantic shift refines no_progress heuristic" — explicitly acknowledged in preregistration Appendix E header that primary §2.5 FE gate operates on episode-level `success` (unchanged) while step-level heuristic refinement is the precise reason Phase 1a cold re-fire is required. (e) "claimed B-554-style cohort stratification but code is column-level" — corrected above: about_blank_frequency.py does column-level attribution split (within-row sub-columns), distinct from B-554 row-level cohort isolation.

**Gate 8 is an investigation gate, NOT an auto skip-list (P2-2, /stress GRL audit 2026-05-20)**: when the same `(site, task_id)` quarantines across ≥2 distinct fires (`quarantine_registry.py:detect_recurrent_failures`, default `min_recurrent_fires=2`), the next paper-grade fire HALTS — independent of per-fire classification status (a `transient_drift` event recurring is no longer transient). The halt is cleared ONLY by one of: (a) a human classification event after matched-temporal-context reproduction (Wave 4 M7), or (b) an **evidence-gated `resolution` event** (`is_recurrence_resolved`) that, per distinct error-class, re-verifies a diagnostic-replay episode (`diagnostic_replay=True` + `sr_excluded=True` + `needs_reevaluation=False`) AND binds the fix to git ancestry (`resolved_by_commit` ∈ [floor, HEAD], re-checked at preflight, not just write-time). The eval-goto class is self-evidenced (`eval_context_mode=isolated_program_html_context`); the screenshot class is bound to its producer commit (≥ C1b `a211ec5`) and labeled `architectural_containment_c1b` (B-1779 — honestly: C1b makes the load-dependent timeout non-fatal, it does NOT prove the substrate no longer times out, so this is containment not temporal reproduction). **Boundary disclosure**: the C3a auto-resolution path moves the task-inclusion decision from pure-human-sign-off to code-checked evidence; we disclose this explicitly — a recurrent-failure task re-enters the canonical SR denominator only when the deployed code carries the fix AND a matched-context diagnostic episode is on record, never by classification-status alone or silent skip. Repeatedly-unresolvable tasks stay HALTED (fail-closed), never silently dropped.

Cross-link: 实验笔记 §237 (full RCA chronicle) + §241 (Fire-6 Stage C) + §247 (GRL boundary audit), preregistration Appendix E (additive disclosure framing + invalidation thresholds + terminal mapping), master_bug_catalog P0-1-ABC* / P0-8-C* / P1-10-B / P1-11-B* / B-1764 (Rule 2) / B-1770~B-1773 + B-1779 (resolution mechanism).
...
**Cost attributes the sink to `wasted`; latency does not subtract it — §4 decomposition asymmetry only (B-1842, Claude Mode A + codex Mode B 2-AI overlap).** The three-column cost (`compute_three_column_cost`) routes a parse-error / injected-wait step's spend to `protocol_wasted_cost` (excluded from `canonical_action_cost`). The parallel canonical *latency* axis (`scripts/analysis/aggregate_cross_site.py:309`, `avg_total_latency_canonical_ms = minus_retry − busy_wait − recovered`) has **no** symmetric parse-error-sink subtraction term, so the sink step's inference latency remains in canonical latency. **The §1 headline is unaffected**: the §1 cross-baseline cost uses `total_billed_cost` (includes sink spend) and the §1 latency uses `total_latency_minus_retry_ms` (includes sink latency) — both *include* the sink, so the headline is symmetric. The asymmetry is confined to the §4 within-episode efficiency *decomposition*, where canonical-cost excludes the sink but canonical-latency retains it. Forward remediation: add a `− parse_error_injected_wait_ms` term to the canonical-latency composer (queued post-fire); given the ≈0 sink rate the magnitude is empirically negligible.

**Canonical-action validity keys on parse-validity, not environment outcome (B-1844, codex Mode B OOB).** `classify_step_accounting` defines `valid_agent_action = parse_valid AND not wait AND not goto_blocked` (`metrics.py:441`) — it does **not** consult `action_success` or `page_changed`. A parseable-but-semantically-empty action (the local 4B failure mode "always emits *some* JSON" — a click on a stale id, a no-op type) is therefore counted as a canonical action: it consumes the agent-action budget and its spend lands in `canonical_action_cost`, whereas a B0 structured-emission failure routes to the wasted bucket / parse cap. This is a confound in the *opposite* direction to the cap-termination interaction in §3.5 (which disadvantages high-parse-error baselines): parseable-but-wrong behavior advantages a model that fails *fluently*. `no_progress` is recorded separately (`runner/main.py:3918`) but does not gate validity. Forward remediation: report `no_progress_rate` per cell as a covariate alongside `parse_error_rate` (disclosure-level; we deliberately do NOT add env-outcome to the validity predicate, which would diverge from upstream "N agent decisions" budget semantics).

**WAIT-rescue soft-retry is not strictly state-neutral (B-1845, gemini Mode C OOB).** The soft-retry mechanism re-observes "the same page" after a rescued WAIT, implicitly assuming the observation at retry equals the observation at the failed step. On a live site this can fail: time-dependent content (relative timestamps "posted 3 hours ago", lazy-loaded elements, rotating banners) means the retry observation is taken at t+ε, not t₀, weakly perturbing the trajectory's Markov property and exact reproducibility. The VWA sites are self-hosted Docker snapshots (largely static), bounding the magnitude, but we disclose that a WAIT is not a guaranteed no-op observation. No remediation beyond disclosure.

Two further forward-instrumentation gaps were logged for post-fire: an explicit `termination_reason` episode field so cap-induced early termination is directly readable rather than inferred from `parse_error_injected_wait_count == max` (B-1846), and a condition-level B0 `tool_call_emit_rate` gate in `paper_grade_check.py` so a silent B0 fallback to the text-parse path cannot pass the parse-symmetry check undetected (B-1847; empirically emit = 100% on the canonical substrate, but currently un-gated).

Cross-link: §3.5 Protocol Reset accounting paragraph (cap interaction + B0 ≈ 0 % / B1 0 % / B2 0.7 % rates) + master_bug_catalog B-1842 / B-1843 / B-1844 / B-1845 / B-1846 / B-1847 + 实验笔记 §278 (3-AI /stress chronicle).
