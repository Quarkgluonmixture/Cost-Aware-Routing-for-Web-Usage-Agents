...
| `2c15d66` | C1b screenshot-timeout non-fatal recovery | On a heavy-DOM page where `Page.screenshot` exceeds 30s, `AsyncScriptBrowserEnv.astep` returns a blank ndarray + `info['screenshot_timeout_recovered']=True` instead of raising (`content()` captured first). Mode-AGNOSTIC at the submodule; the dom-only vs som/vision fatality decision lives in P79 `VWAWrapper._gate_screenshot_timeout`. **Behavioural impact**: dom-mode (artifact-only screenshot) episodes no longer quarantine on the Fire-4 task-75 `agent_observation Page.screenshot` timeout class; som/vision/phantom_* (decision-input) re-raise as before; screenshot still attempted every step → cross-mode latency comparability preserved | `browser_env/async_envs.py` (+ P79 `p79/envs/vwa_wrapper.py`, `p79/experiment/runner/main.py`) | §4.X.11 row, §3.5.1, prereg §7 (SBOM re-lock) |
