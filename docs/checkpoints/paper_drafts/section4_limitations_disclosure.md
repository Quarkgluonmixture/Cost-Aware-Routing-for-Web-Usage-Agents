...
Mixed pre/post-Phase 2 archive aggregation requires stratification by `intervention_type IS NULL` (legacy cohort) vs `intervention_type IS NOT NULL` (post-hardening cohort), sibling pattern to B-554 evaluator_authority_mode archive cohort isolation. Pre-fix Fire-3 partial archive (75 cls episodes, aborted at task 75) is NOT used for canonical claims; Phase 1a re-fire post-hardening is the primary substrate.

**B-486 quarantine event rate (P0-8-C* Gemini OOB)**: when an episode's evaluator phase exhausts 3 retries on Playwright navigation timeout / dep exception / OpenAI 503, the runner raises `EvaluatorUnavailableError` (`environment.py:285-422` per B-486 paper-grade fail-loud contract) and aborts the condition without writing `condition_summary_v2.json` for the affected episode. The quarantined episode summary carries `needs_reevaluation=True`; downstream watchdog respects this field (`experiment_watchdog.py:1789-1815` P1-10-B fix) and does NOT auto-retry — manual operator review is required to re-fire.

Cross-baseline asymmetry is expected: B0 (AWS proxy, ~24s/step latency) has higher Playwright timeout exposure than B1/B2 (local 4B inference, ~2-5s/step) at high task count substrates. Phase 1a re-fire will measure per-cell `quarantine_rate = quarantined_episodes / scored_task_count` as an Appendix Sensitivity column. The Fire-3 cls B0 DOM 2026-05-19 00:53 quarantine on task 75 was an empirical anchor for this attack vector; Probe A (B1 task 75 isolated) + Probe B (B1 tasks 70-75 prefix) both reproduced zero eval timeout, indicating the gap is B0/substrate-scale-specific (75-task dwell time × ~24s/step ≈ 15h vs B1 Probe B 5-task × ~2s/step ≈ 10 min — 4 OOM dwell-time differential not directly testable without cost-prohibitive B0 prefix replay).

**Reviewer attack defused**: (a) "page_changed conflates agent + framework" — page_changed now agent-only; intervention separately attributed. (b) "B0 quarantine triggers more frequently than B1/B2, silently inflating SR" — per-cell `quarantine_rate` as Appendix column; watchdog preserves quarantine evidence (no silent re-roll); `needs_reevaluation=True` requires manual operator review. (c) "cross-baseline asymmetric scaffold effects undisclosed" — explicitly disclosed here as Limitations.

Cross-link: 实验笔记 §237 (full RCA chronicle), preregistration Appendix E (additive disclosure framing), master_bug_catalog P0-1-ABC* / P0-8-C* / P1-10-B / P1-11-B*.
