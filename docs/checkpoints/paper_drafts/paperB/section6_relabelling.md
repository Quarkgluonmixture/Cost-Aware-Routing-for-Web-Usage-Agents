...
model identity (§2.3), so two backbones facing the same task present the same vector. Of the
tasks labelled in two or more cells, **57.4%** on classifieds and **56.0%** on reddit carry
conflicting labels, capping any task-feature classifier at **79.2%** and **83.7%**
(Appendix A.4). Pooling is less a mistake than a change of question: it estimates which mode is
best marginalising over backbones, and a deployment has one backbone and needs the answer for
that one.
