...
<!-- Generated by scripts/analysis/export_ablation_tables.py. Do not hand-edit between the markers. -->

> **Multiplicity status of this table set.** These 42 tables are **not one inferential family** and are not corrected as one. Read them in three classes:
>
> * **Confirmatory** — hypotheses fixed before the data, corrected within their own family. Currently: the 2×2 axis independence set (64 conjunction hypotheses, BH/Holm on `max(p1, p2)`; see that table's caption). *Which further tables join this class is a framing decision and is still TBD.*
> * **Descriptive** — quantities reported with intervals but no test, and no correction claimed: the full behavioural matrices, the efficiency tables, the failure-attribution counts.
> * **Selection-derived (exploratory)** — a maximum or argmax over layers, signals, thresholds or comparators is part of the statistic. These bound what a selection could deliver and **cannot be read as effect estimates**: the cascade control (best of 8–10 signals), the linear-probe AUROC (best of 37 layers), the reference-image and visual-difficulty diagnostics (selected comparators). Each such table states it in its own caption.
>
