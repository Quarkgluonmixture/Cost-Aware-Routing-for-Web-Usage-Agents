...
for argument in "$@"; do
  case "$argument" in
    paperA|paperB|realm) PAPER="$argument" ;;
    --submission) SUBMISSION=1 ;;
    -h|--help)
      printf 'Usage: %s <paperA|paperB|realm> [--submission]\n' "$0"
      exit 0
      ;;
    *)
...
if [[ -z "$PAPER" ]]; then
  printf 'ERROR: paper id required (paperA, paperB or realm)\n' >&2
  printf 'Usage: %s <paperA|paperB|realm> [--submission]\n' "$0" >&2
  exit 2
fi
...
case "$PAPER" in
  paperA)
    ...
    ;;
  realm)
    # Rewrite skeleton, 2026-08-03. Sections 3-5 are intentionally empty: the claim is
    # chosen against the evidence, not the other way round, and three frames died in three
    # days from writing prose first. section1_intro carries the evidence inventory in the
    # Abstract slot; the guide and the tables are generated by
    # scripts/analysis/export_ablation_tables.py and must not be hand-edited.
    SECTIONS=(
      section1_intro.md
      section2_setup.md
      section_evidence_guide.md
      section_evidence.md
      section3.md
      section4.md
      section5.md
