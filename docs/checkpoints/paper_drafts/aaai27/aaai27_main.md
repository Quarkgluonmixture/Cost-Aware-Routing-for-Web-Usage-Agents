...
VERDICT SLOTS (data-conditional per prereg R1–R5; do not pre-commit):
  <H1-VERDICT>  FE pooled P-SoM drop-one vs +1.0pp — currently
                PARTIAL_DATA (k=3 complete cells; final requires k=6)
...
Two readings matter. (i) The six-mode ceiling (43.3%) is far above the best single arm (27.2%): 36 of 97 oracle-solvable tasks lie outside the best single arm — the existence proof for representation routing on this cell. (ii) Every arm, including all three phantom arms, carries a positive point-estimate of irreplaceable coverage, with P-prompt's CI excluding zero here. The other two complete cells repeat the pattern at lower amplitude: P-SoM drop-one +1.34pp [0.00, 3.12] on cls·B1, +0.45pp [0.00, 1.34] on the cls·B2 floor [A]. The pre-registered hero gate is *not* a per-cell statement: H1 pools the P-SoM drop-one across all six cells by fixed-effects inverse variance and tests it against +1.0pp. <H1-VERDICT: PARTIAL_DATA as of 2026-07-14 — interim pool over the k=3 complete cells (all classifieds) gives θ_FE = +0.98pp, primary paired-bootstrap 95% CI [0.24, 1.83] (normal-Z transparency CI [−0.05, 2.00] is the appendix channel, NOT the gate; PROTOCOL_NOTE_05 pipeline), point estimate *below* the +1.0pp bar on classifieds alone; the k=6 verdict hinges on the three pending Reddit cells (archive calibration [V]: pooled +2.34pp, I²=0%, with Reddit the strongest P-SoM site). Populate via `verdict_day_slotsheet.py` when `analysis_status=COMPLETE`; do NOT quote the interim pool as the verdict.>

The structural gates ask whether the space is genuinely two-dimensional: H3(i) tests whether P-text solves tasks P-SoM does not; H3(ii) the same for P-prompt. <H3-VERDICT: PARTIAL_DATA as of 2026-07-14 — interim FE pools over ALL landed complete cells per the locked estimand (PROTOCOL_NOTE_05 corrected an outcome-dependent cell deletion; the superseded 2026-07-01 axis-1 figure of +3.20pp [k=2] must not be quoted): axis-1 +1.08pp, paired-bootstrap 95% CI [0.47, 1.98] (k=3; the one noise-floor cell affects cell-level pass only, not pool membership) and axis-2 +2.26pp [1.23, 3.46] (k=3), both interim CIs excluding 0; final verdict at k=6.> On cls·B0 [A], both axis unique-sets are non-empty under the pre-registered estimand: P-text solves 9 tasks P-SoM misses (4.02pp) and P-prompt 16 tasks (7.14pp) — pairwise non-redundancy against the compound arm, a distinct and looser quantity than Table 3's drop-one irreplaceability against the full portfolio.
...
**Statistics.** The design is powered for medium pooled effects, not small per-cell effects; the pre-registration's archive power projection used a 4-mode additive estimand that upper-bounds the strict 6-mode gate effect (Amendment 02), so strict-gate power is reported as <TBD> rather than that projection. Per-cell CIs are transparency, never gates. Fixed-effects pooling over the six planned cells deliberately diverges from small-k HKSJ recommendations *on design grounds* (the cells are the population); HKSJ sensitivity is in the appendix, and per-cell weights use bootstrap SEs under a pre-registered 1.0pp floor (Agresti-Coull-anchored), so a degenerate low-variance cell cannot dominate the pool. H1 is the single primary statistical test; H10 is an operational gate with no across-cell α. The pre-registration disclosed a joint Type-I exposure of 0.0975 over {H1, H10} prior to H10's reclassification as an operational criterion; we retain that disclosure for transparency while noting that no joint FWER is defined under the current H10 semantics.
