...
Two further properties make the space *deployable* rather than merely curious — both architectural consequences of the screenshot-off boundary, not empirical discoveries. First, **cost ≈ DOM by construction**: `[SOM_MARKS]` is a regex filter plus deterministic renumbering over the *same* accessibility-tree text DOM already consumes, with no page-screenshot tokens and identical reference-image payloads; we pre-registered a falsification check (per-task median cost ratio vs. DOM ≤ 1.20× per cell) rather than a category-error equivalence test. Second, **latency below full SoM**: the arms skip per-step marked-screenshot encoding (archive cls P95 step latency 18.2s P-SoM vs. 74.0s SoM [V]; canonical recomputation in §5.4). Together they position the phantom arms as additions to the routing *menu* at DOM-equivalent cost — exactly what a representation router needs.
...
**Routing in LLM systems.** Existing efficiency routing operates on the *compute* axis: which model [@chen2023frugalgpt; @ong2025routellm; @webrouter2025], which capacity within an agent [@li2026pando], which numeric precision [@li2026dmr], which grounding expert [@li2026avenirweb], or how to schedule modality encoding in serving [@qiu2025modserve]. A recent survey spanning six routing paradigms confirms the pattern: every paradigm selects *which model* answers, and observation-representation routing is absent [@moslem2026routingsurvey]. Agent-E selects among DOM-variant text views, but within one text modality and not learned per task [@dhondt2024agente]; adjacent non-model levers also stay off our axis — difficulty-triggered visual-detail escalation within one mode [@wang2026lazymcot], routing between direct inference and a full agent pipeline [@wang2026boundaryrouter], and retrieval-pipeline tiering, where a per-query oracle dominates every fixed choice yet a learned router recovers it only partially [@genc2026adaptivereranking], an oracle-vs-router gap we meet again in §6. Our router is, to our knowledge, the first *learned, per-task representation router within a single model* that crosses the screenshot-on/screenshot-off boundary. These axes compose rather than compete: a production stack can route representation first and escalate models on stall; and even under security-motivated plan-then-execute architectures, whose runtime subroutines still consume page observations [@piet2026plan], the representation choice persists.
...
**Efficient observations and language priors.** FocusAgent prunes accessibility-tree text with a retriever [@kerboua2025focusagent], and observation-reduction benchmarking systematizes such *within-representation* compression [@enomoto2026observation]; ReVision prunes visual tokens with a learned keep-mask [@abaskohi2026revision]; transport-layer measurements find that the structured text, not the pixel stream, carries most of the UI-task signal [@meng2026sema]. Phantom arms are none of these: they *re-format the same text content* (within ±7% of DOM token length [V]) and drop the per-step screenshot entirely. Their viability is made plausible by evidence that VLMs often answer from language priors [@asadi2026mirageillusionvisualunderstanding; @vu2026scaffold; @zhou2026visualignorance], that late-layer decisions are dominated by language-unique information with task-dependent layer fingerprints [@wu2026visionbecomeslanguage], and that the bottleneck is often integration rather than perception fidelity [@liu2025seeing] — motivational anchors only; none is web-agent evidence, and we import no mechanism claim from them. Where follow-up work treats the image-free regime as a failure mode to detect and abstain from [@chowdhury2026miragedetect], we treat it as routing structure to exploit.
...
**Evaluation.** H10 is a two-layer *operational deployment gate*, not a significance test: a cell passes if the router's (Cost, SR) point is Pareto non-dominated by all five pre-registered single-mode baselines in ≥95% of task-paired bootstrap replicates; the router is *deployable* if ≥5 of 6 cells pass. An intelligent-baseline ladder bounds the router from both sides: always-cheapest, decision stump, DOM-features-only LR, and a per-task lookup table (an ∞-capacity reductio bounding generalization headroom). On cls·B0 the ladder spans always-DOM 17.4% → stump 19.6% → 8-feature LR proxy 25.0% → oracle ceiling 43.3% (disclosure-only: the proxy is an archive-sanity stand-in entering no gate); it brackets the plausible value of task features (+5.4pp over the stump) and the headroom no 18-feature router can close (+18.3pp to oracle). Following the construct-validity protocol of [@li2026aucnotenough], we additionally report a trivial-covariate scalar baseline (site, intent length, and template-surface statistics only) and a template-disjoint split sensitivity, so that *learnable from task features* cannot be conflated with *memorizable from template identity* \<TBD: populate from `router_covariate_baseline.py`\>.
...
**Representation as a deployable control surface.** Three properties make the phantom arms deployable levers rather than benchmark curiosities: model-fixed (no second model to host), training-free at inference (a regex over text the agent already consumes), and cost-anchored to DOM by construction. The empirical content sits on top: low-overlap success pools (§5.3) with irreplaceable oracle coverage (§5.2) — what a router can monetize.

**Composition with compute-axis routing.** Representation routing is orthogonal to model/capacity/precision routing and to token pruning; a production stack can route representation per task, prune within the chosen representation, and escalate models on stall. We resist the stronger reading that it *replaces* model routing, and register a boundary condition: the lever's magnitude tracks the per-step cost of image encoding, and cheaper vision encoders will shrink (not erase) it.
