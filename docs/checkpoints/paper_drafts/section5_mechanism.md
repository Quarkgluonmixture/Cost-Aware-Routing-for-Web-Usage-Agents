...
**Evidence status (added after /codex-stress 2026-05-12, before any reader reaches §5.2)**:

| Evidence layer | Method | Status |
|---|---|---|
| Geometry (cosine PCA per layer) | Method 4.2 (§5.2) | **Strong** — AUROC 1.0 on key contrasts, 5/5 robustness, 100% per-task above 0.010 threshold |
| Causal continuation patching (SoM → no-image arms) | Stage 2/3 (§5.4) | **Causal** — mid-layer L12-L18 transfers across cls + reddit, additive across DOM/P-text/P-prompt targets, Gaussian-random negative controls at ~0 |
| Causal axis-2 prompt-only patching | Exp 5 cellhprompt (this section) | **Causal continuation evidence on 2 sites N=24 each**; bootstrap CI and content-matched (task-shuffled / random-source) controls **PENDING** (cellhprm_*_rand 359719/359720 in flight) |
| Steering (mean-diff activation) | Method 4.4 (§5.3) | **Weak / partial** — best H-mean 0.33 at L33 α=10, layer-α tradeoff prevents single sweet spot, treated as evidence ceiling not validation |
| Output divergence | Exp 3 logit lens (§5.7) | **Geometry-amplification at L23-L25**, NOT a deployed classifier; deployment claim deferred |
