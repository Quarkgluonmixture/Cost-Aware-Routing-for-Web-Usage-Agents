...
## ✅ Phase 1 完成 2026-07-28

五批全部落盘，合并阶段完成，总索引在 `INDEX.md`。

合并阶段实际做了什么：
- **flag 语义三分**：删 30（paper 章节号撞号污染）· 反转 17（点名者自身已作废 ⇒ 被点名者是反证）· 保留 117 待判
- **§299.4 归属确认**：在 §397.10 名下，非遗漏
- **跨批作废回标：尝试后放弃** —— 特征数字自动匹配 77 处命中里绝大多数假阳性
  （`2026` 是年份 · `4096` 撤的是"造成 KV 开销"这个假设而非配置值 · §120 原文明说两数各自正确）。
  已作为已知限制写进 `INDEX.md` §3，替代方案 = 用数字前先 grep `retracted.md`
- **INDEX.md**：五批导航 + 按问题找 + 5 条已知限制 + 已裁悬案 + 5 条演化链

未做且不打算做的：矛盾不调和（设计如此，留在各批文件末尾，合计 ~93 条并列）。
