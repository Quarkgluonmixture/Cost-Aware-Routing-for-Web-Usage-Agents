...
4. **`adjusted_success` retired + `EXPECTED_N` sweep** — *(4a + 4c DONE 2026-05-14; 4b + cosmetic 4d remain)*.
   ...
   - **Remaining**: (4d, cosmetic) graceful-fallback readers still emit `adjusted_success` columns as `== success` aliases — full column removal is a separate low-priority pass, not a correctness item.
