...
4. **`adjusted_success` retired + `EXPECTED_N` sweep** — *(partial 2026-05-14)*. **Done**: `scored_task_count(site, benchmark)` single-source helper added to `analysis.py` (total tasks − N/A excluded; verified all 6 sites — cls 224 / red 205 / shop 435 / wa-shop 173 / wa-admin 176 / wa-red 104); the 3 fire-critical **live-monitor** `EXPECTED_N` consumers switched to it — `glm_cell_autoupdate.py` (cron DONE-gate — was the Phase-1a "cell stuck pending" risk), `run_registry.py` (`is_complete`), `active_processes.py` (`make active`). **Remaining**: (a) the ~6 non-live `EXPECTED_N` consumers (`power_analysis.py` / `mechanism_per_task.py` / `axis_effect_size.py` / `axis1_microbehavior.py` / `fig1ab` / `fig3_regional_carbon`) need per-file judgement — some analyse *archived* (pre-exclusion) data and may legitimately keep 234/210/466; (b) the ~27 `adjusted_success` / `fp_reason` reader files — decision locked "统一切 `success`" but the sweep needs per-file judgement (paper-grade reader vs Appendix-D archived reader vs already-has-`success`-fallback). Foundation laid; the judgement-heavy sweep is a focused follow-up pass.
