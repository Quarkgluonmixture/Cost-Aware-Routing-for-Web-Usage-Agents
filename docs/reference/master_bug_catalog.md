...
### Tier 3b reserved (B-910 + B-911, monitoring quality) — pending commit

- **B-910 P1-11-B***: `glm_cell_autoupdate.py:208-220` latest_match in-flight prefer when mtime newer than finalized (re-run window correctness)
- **B-911 P1-12-B**: `active_processes.py::Cell.runner/watchdog` → list, render `DUPLICATE-RUNNER(n=2)` + all PIDs (multi-session collision detection visibility)
- B-912~B-915 reserved for Option C sentinel infra `logs/launching/${benchmark}_${site}.json` (if landed)
