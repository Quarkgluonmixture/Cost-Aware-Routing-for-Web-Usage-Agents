...
- **B-103** Mechanistic extractor `_build_user_text` missing `Accessibility Tree:\n` prefix for DOM / phantom_prompt — paper-§5 NPZ byte-identity claim broken for these 2 modes. **NOT FIXED**: mechanism §5 paused per advisor §137+§138; code change 5 min but touches mechanism file (`p79/mechanistic/extract_hidden_states.py:85-100`) + 2 cross-family scripts copy same divergence. Archived NPZ for these modes already invalid; full fix requires re-extract on §5 re-activation. Codex Mode B (mechinterp implementer persona) caught this in trace mid-investigation; Claude missed.
- A1.4 Claude F1 cross-family `_options_map` skip / F3 inject window / F4 `_collect_bbox_map` recursion / F5 `apply_som` legacy / F6 `_options_map` telemetry — all design-time / mechanism-paused-area latent, catalog backlog.
