...
Smallest reportable p at B=10000 is 1/(B+1) = 1.00e-04; Holm's tightest threshold over 8 cells is 0.05/8 = 6.25e-03. B is therefore not what decides any cell's verdict (it was at B=200, where the floor 4.98e-3 sat inside the threshold and the surviving cell reported exactly it).
...
Holm at α=0.05 over the m=8 cells tested (the sweep was run once per cell, so the family is those 8 cells) — **1 of 8 reject**:
...
Read together — and note this is a **narrower** negative than an earlier draft of this file claimed. Across the 8 cells the label's AUROC spans 0.526-0.758, and unlike the which-mode task the LR clears the best single covariate in 5 of 8. 1 cell yields no SR-lossless saving at all; in 2 cells a signal-free pipeline reproduces at least 80% of the saving under shuffled labels. **One cell, reddit·B2 (p=0.0004 vs 0.0063), has a saving that survives Holm at m=8** — under the corrected bundle-permutation null; the earlier y-only null reported 0.040 for reddit/B2 and supported a blanket 'nothing survives' claim, which was wrong.

⚠️ **The cell that survives is reddit·B2, and its AUROC is 0.526** — below its own best single covariate (0.790). That is not a contradiction: the two quantities measure different things, and on this data they come apart. AUROC scores the GLOBAL ranking; the saving comes from the TAIL. reddit·B2 sends 192 of 203 tasks (95%) to the cheap mode with no SR loss — in a cell where only 7.4% of tasks are solvable at all, almost nothing in that 95% was ever going to succeed. It differs from the free always-cheapest policy by 5% of the task allocation, and those 11 retained tasks happen to hold 5 of the successes (9 vs 4). The permutation null is detecting that tail enrichment, not a globally ordered score.
So the honest phrasing is NOT 'the label is predictable, yet triage fails'. It is: **at 2-36% base SR, a high AUROC is neither necessary nor sufficient — what decides whether triage saves anything is whether a handful of tail tasks land on the right side, and at n=203 that handful is 4 successes.**

What still holds, and is the load-bearing statement: **no cell's learned triage Pareto-beats the trivial always-cheapest fixed policy** (0 of 8). In reddit·B2 specifically the learned policy keeps +2.46pp SR against always-cheapest while paying +1.7% cost — a genuine trade-off point, not a dominating one, and not something a deployment would prefer without a stated SR price. So: a detectable signal in 1 of 8 cells, worth less than the policy you get for free.
...
Contrast with the which-mode half: that one fails on label SUPPLY (15-97 labels per cell on the six VWA cells, 笔记 §383.4 — that count comes from `router_objective_ordering`, not from this file, so it is not re-derived per cell set here). Triage has the labels and the AUROC and still does not beat a fixed policy — a different failure mode, at 2-36% base SR where almost every task is hopeless and 'always take the cheap one' is already close to optimal.
