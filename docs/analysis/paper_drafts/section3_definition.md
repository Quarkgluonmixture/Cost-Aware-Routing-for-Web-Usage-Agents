...
Phantom-SoM is also a cost intervention, but the savings come from a different place than one might assume. We can decompose the SoM bundle into three potentially expensive layers: (1) the text observation, (2) the on-server annotation pipeline that extracts element boxes and draws numeric labels onto a screenshot, and (3) the image tokens that the marked screenshot contributes at inference time. Phantom-SoM saves on layer (3) by construction: no marked image is sent to the model. Layers (1) and (2) are essentially unchanged. The text observation has comparable token length to the corresponding AXTree (within ±7% on both sites: median total input 3437 vs 3661 on reddit, 3008 vs 2948 on classifieds, holding the system prompt fixed at the DOM family); the difference is in structure (flat indexed list versus nested hierarchy with url/tab metadata), not in length. The annotation pipeline still runs in our implementation, because the marked screenshot is kept on disk for debugging even when the agent does not see it (median obs-prepare latency stays at ~30 ms per step, comparable to full SoM); a deployment-time variant could omit the image draw to save an additional ~30 ms and ~$2e-5 per step, but that contribution is small relative to inference cost. The dominant saving from Phantom-SoM is therefore the image tokens themselves, which we estimate at roughly 600 per step on reddit and 1100 on classifieds under our backend tokenization. Section 4 tests whether this image-free condition also creates independent routing value.
