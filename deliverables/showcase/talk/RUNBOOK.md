# Runbook —— 演讲那 10 分钟，手要做的（16 Sep · 下午学生演讲第二个，约 14:55）

_只放操作：环境、标签页、别碰清单、出事怎么办。要念的在 `rehearsal-script.md`，展板 live 页的运维在 `../demo/README.md` → *Live*。_

## 开讲前准备（14:35 带 MacBook 从展板走到讲台；你是第二个讲）

_演讲只用 **MacBook + Chrome**。quark 全天留在展板（live 页的 ssh 隧道只在 quark 上能开），不要搬它。_

_你是下午第二个讲（09-15 user 告知，ROADMAP D19）：第一位讲者用着投影时先做第 2–7 步，换场时再做第 1 步接投影，并回头确认第 4、5 步。早上没赶上 AV check（user 09:50 才到）→ 14:35–14:45 趁第一位开讲前接一次投影试镜像。_
...
| prize | 两块数字卡（27 → 43 · −20%），静态；念完按 → 到 `question` |
