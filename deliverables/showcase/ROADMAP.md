...
- **D26（09-15 深夜，user）agents 页改成「大 logo + 裁到表单的抓取」，BOTH 换 browser-use。** user 看 D24 版截图：三块里的注册页太小、留白多；框里的 CSS 标题栏认不出工具；「不一定要完全真实，可以合成，logo 大一点更好认」。落地：三块各一个 7vh 的 logo（`talk/codex-color.svg` / `claude-color.svg`，browser-use 用 GitHub 组织头像）+ 工具名 + 方法一行（computer use / Playwright MCP / highlighted elements）；下面的抓取都裁到表单区（视口 30,55–540,600）；BOTH 从「our agent · 我们画的 25 框」换成 **browser-use 0.13 自己对同一页的高亮截图**（`talk/browser_use_capture.py`，48 个元素，它默认 `highlight_elements=True`）—— 台下认识 browser-use，而且它从机制上就是 SoM 家族；VisualWebArena / SeeAct 只在被问到时说。user 不必再截终端图。口径：logo 只标工具；demo 里 BOTH 的青色框是我们的，说「同一类做法」。
