...
| `+11 to +16` · `−14 to −27%` · `−7 to −29%` · `up to 34%` (talk `prize`, 09-15) | same source as the row above (`talk/hindsight_efficiency.json`, B0 on classifieds / reddit / WebArena reddit, perfect router vs best single view): success +16.07 / +11.33 / +16.35; token bill (billed cost) −20.2 / −13.7 / −26.9%; token-estimated CO₂e −14.4 / −7.3 / −28.6%; time per task +0.5 / −2.0 / −33.9% | a **perfect-router** bound — say "if it always picked the right view". CO₂e is estimated. Time is faster on **one** site only, the same on two: never "34% faster" alone. Not a Claude or GPT measurement; the `agents` slide before it is an illustration, and GPT-6 Astra's own interface is not in our records (the 09-09 survey documents OpenAI's API computer use as screenshot-only through GPT-5.4, and Codex Desktop as hybrid) |
| `161` lines · two real captures (talk `agents`, 09-15) | `talk/fig/real_wiki_createaccount_snapshot.md`: the real Playwright MCP server (`@playwright/mcp` 0.0.81, server 1.64.0-alpha-2026-09-14) `browser_snapshot` of en.wikipedia.org `Special:CreateAccount` (redirects to auth.wikimedia.org), viewport 1280×800, 15 Sep 2026 — 161 YAML lines / 7,691 bytes; the screenshot is `browser_take_screenshot` in the same session (`talk/real_capture.py`) | one page, one capture; the page changes. Lines, not tokens. Neither image is a Claude or Astra run. The Astra side rests on OpenAI's computer-use docs: the model "uses screenshots and other tool results to decide what to do next" |
