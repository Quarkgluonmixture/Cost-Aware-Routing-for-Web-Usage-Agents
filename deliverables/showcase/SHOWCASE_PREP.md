...
| browser-use highlighted capture · `48` elements (talk `agents` BOTH panel, D26 09-15 late) | `talk/browser_use_capture.py`: browser-use **0.13.10** (`pip install browser-use`, a scratch venv), headless Chromium 1280×800, `highlight_elements=True` (its default), same URL; `get_browser_state_summary` gave 48 interactive elements, then its own `create_highlighted_screenshot_async` drew its dashed boxes + index labels → `talk/fig/browser_use_wiki_highlighted.png`; its text side (`[164]<nav …>` lines) → `browser_use_wiki_text.txt`. The slide shows the crop `browser_use_wiki_form_highlighted.png` (viewport box 30,55–540,600) beside the same crop of the MCP screenshot (`real_wiki_form_screenshot.png`). Logo = the browser-use GitHub org avatar (`fig/browser-use-logo.png`); Claude / Codex logos are the user's SVGs | real: browser-use's own drawing on its own screenshot — say "browser-use does this by default". It is not an agent run and not our benchmark's marks: the demo's BOTH lane draws **our** cyan boxes (VisualWebArena-style SoM); say "the same kind of view", never that they are the same tool. `filter_highlight_ids` hides indices on elements with long text, so some boxes carry no number — don't call every box "numbered" if pressed |
