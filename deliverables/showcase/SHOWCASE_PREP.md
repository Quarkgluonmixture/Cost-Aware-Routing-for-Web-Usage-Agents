...
| `+11 to +16` · `−14 to −27%` · `−7 to −29%` (talk `prize`, 09-15; **page 2, before `agents`**, D24) | same source as the row above (`talk/hindsight_efficiency.json`, B0 on classifieds / reddit / WebArena reddit, perfect router vs best single view): success +16.07 / +11.33 / +16.35; token bill (billed cost) −20.2 / −13.7 / −26.9%; token-estimated CO₂e −14.4 / −7.3 / −28.6%. The time card (`up to 34%` on one site, +0.5 / −2.0 / −33.9%) came **off the slide** 09-15 evening (D24): it hedged the hook | a **perfect-router** bound. The slide comes before "view" or "router" is introduced, so it says "the right way to see each page" and the foot says "upper bound: the right choice on every task"; the script names it *perfect router* only from `learned` on. CO₂e is estimated. Time, if asked: faster on **one** site only, the same on two — never "34% faster" alone. Not a Claude or GPT measurement; the `agents` slide after it is an illustration, and GPT-6 Astra's own interface is not in our records (the 09-09 survey documents OpenAI's API computer use as screenshot-only through GPT-5.4, and Codex Desktop as hybrid) |
