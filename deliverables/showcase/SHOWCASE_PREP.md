...
| `1 of 8` | perfect-hindsight choices that win (the poster's word for the oracle; the talk says **perfect router**, D21) | same bar — this line is mandatory, never quote 0/8 alone. The strip labels say **won on both success and cost**; +16.35 is **success only** |
...
| `about 30%` vs `about 6%` | share of steps spent **scrolling** (talk slide 4; poster panel 3): image-only median 29.7% (range 24.1–36.0) over the 8 settings; the other five views' medians 5.6–6.9% (`per_mode_four_dimension_profile_with_wa.json`, `scroll_frac`) | **median over 8 settings**. Image-only's lowest setting (24.1%) is above every other view's highest (20.8%), so "scrolls most in every setting" holds — but per setting the gap is 1.25–7.05×: never say "4× in every setting" |
| `~4×` | poster panel 3 caption *Vision scrolls ~4× more*: image-only's median scroll share over each other view's median, 4.3–5.3× | medians, not per setting (row above) |
| `about half` | share of steps spent **typing**: image-only median 8.9% vs the other views 17.3–22.0% (0.41–0.51×) | median over 8 settings |
| `2.3×` (with 2.2× · 2.2× · 1.6×) | poster panel 4 = REALM Table 41 block A: on the 101 tasks only an image view solved, how often the **text** side's failures show *gives up when not found* 2.31×, *click-back oscillation* 2.25×, *target not in the text* 2.24×, *page is visual, no screenshot* 1.65× — **against how often that same side's failures show it everywhere** | each side against **its own** baseline; never compare text-only with image-only (four text arms vs two image arms). A rule hit is a symptom, not a verified cause: say "shows up 2.3× as often", never "because" |
| `1.2×` · `0.9×` · `0.9×` | Table 41 block B: on the 109 tasks only a text view solved, the **image** side's failures — action loop 1.17×, never paginates 0.93×, budget exhausted 0.91×. The one higher row (3.61×) rests on 8 hits, all in the two WebArena settings, and is left off the figure | "no failure stands out — it fails there the way it fails everywhere" / "it never gets there" (the table's own words: *it did not arrive*). The poster caption *image-only toward stalled progress* is looser than the table; don't repeat it |
| `over 2×` (talk slide 5 foot) · **scope** | the text-side rules above 2× — P17 click-back 2.25× (61 hits), P16 target not in the text 2.24× (25), P27 gives up when not found 2.31× (13); recomputed 2026-09-13 from `conditional_failure_attribution.json` per site | **say "the pattern comes from the six VisualWebArena settings"**: they hold 91 of block A's 101 tasks and every hit of P17 / P16 / P43; on the two WebArena settings (10 tasks) no text-side rule passes 1.73×. Block B on VWA alone tops out at 1.56× (10 hits). P49's pooled 3.61× is 8 hits, all on WebArena, and only **1.53× within WebArena** — pooling inflates it. **Don't lead with 2.3×: it is 13 hits** |
| `LOOK and BOTH` (talk `failure` page) | Table 41's "image channel" is **som + vision** = BOTH + LOOK (`image_modes` in `conditional_failure_attribution.json`); the text channel is dom + the three image-free variants | **never call this side "image-only"** — BOTH carries the text marks too. The poster's `IMAGE-ONLY` label is wrong in the same way |
| `+7.14` vs `+4.46 to +7.59` (talk `hindsight` foot) | B0 · classifieds (n=224), the one setting with every view rerun: adding the best *different* view to the best single view (BOTH) buys 7.14; adding a *rerun* of a view buys 4.46–7.59 (`noise_floor_inventory.json` `margins.cls_B0` / `cls_band`) | arm-count matched: one extra run against one extra run, same setting. **Never** set the five-view gain (`+3.45 to +16.35`) beside a one-rerun band — `noise_floor_inventory.md` §2 lists that as *not licensed*. `2.0–7.6` joins the classifieds band's top to a WebArena pooled-pilot bottom that the same file says is not a floor for READ: **do not use it** |
| `+11 to +16` · `−7 to −29%` · `up to −34%` (talk `hindsight`, 09-15) | B0 (large model) on classifieds / reddit / WebArena reddit, per-task view picked in hindsight (`oracle_sr_cost` picks: cheapest solver, else cheapest view) vs the best single view: success +16.07 / +11.33 / +16.35; token-estimated CO₂e −14.4% / −7.3% / −28.6%; time per task (`total_latency_canonical_ms`) +0.5% / −2.0% / −33.9%; cost −20.2% / −13.7% / −26.9% (`talk/hindsight_efficiency.json`, from `router_objective_ordering.py`) | a **perfect-hindsight** bound (say "if you pick perfectly"); CO₂e is **estimated from tokens** (B0 has no energy counter); time is the same on two sites — never "34% faster" without "on one site" |
| `27 → 43` of 100 · `−20%` token bill · foot `+11 and +16` per 100, `−14 and −27%` · `224` tasks (talk `prize`, **after `demo`**, D25 09-15 late) | `talk/hindsight_efficiency.json`, B0 · classifieds (the demo's site, n=224): best single view 27.23% → per-task perfect choice 43.30% (+16.07), billed cost −20.2% — shown as two bars + one side number. Foot = the other two B0 settings, reddit / WebArena reddit: success +11.33 / +16.35, cost −13.7 / −26.9%. Off the slide: CO₂e (−14.4 / −7.3 / −28.6%, token-estimated) and time (+0.5 / −2.0 / −33.9%). The D24 three-card version (`+11 to +16` · `−14 to −27%` · `−7 to −29%`, page 2) lasted one evening: the user found three ranges before "web agent" was defined both unmotivated and hard to read | a **perfect-router** bound: say "a choice that already knows the answer for every task"; the script names it *perfect router* only from `learned` on. One setting headlines because it is the demo's own site and the other two are on the same slide's foot — never quote 27 → 43 without them. CO₂e if asked: estimated from tokens, −7 to −29%. Time if asked: faster on **one** site only, the same on two — never "34% faster" alone. Not a Claude or GPT measurement |
| browser-use highlighted capture · `48` elements (talk `agents` BOTH panel, D26 09-15 late) | `talk/browser_use_capture.py`: browser-use **0.13.10** (`pip install browser-use`, a scratch venv), headless Chromium 1280×800, same URL; `get_browser_state_summary` gave 48 interactive elements; the slide uses its **DOM overlay** (`dom_highlight_elements=True` + its own `add_highlights` JS: a dashed box and a numbered badge on every interactive element, the look of its early demos) screenshotted by `take_screenshot` → `talk/fig/browser_use_wiki_highlighted.png`; its text side (`[164]<nav …>` lines) → `browser_use_wiki_text.txt`. (Its other rendering, the thin screenshot overlay with indices only on short-text elements, was tried first and read as "not SoM" — dropped.) The slide shows the crop `browser_use_wiki_form_highlighted.png` (viewport box 30,55–540,600) beside the same crop of the MCP screenshot (`real_wiki_form_screenshot.png`). Logo = the browser-use GitHub org avatar (`fig/browser-use-logo.png`); Claude / Codex logos are the user's SVGs | real: browser-use's own JS drawing on its own page — say "browser-use boxes and numbers every clickable thing". It is not an agent run and not our benchmark's marks: the demo's BOTH lane draws **our** cyan boxes (VisualWebArena-style SoM); say "the same kind of view", never that they are the same tool. Its indices are its own element ids (21, 22, 33, 89 …), not 1..K — don't read them as a count |
| `161` lines · `25` marks · three real captures (talk `agents` before D26; the `25`-mark image `real_wiki_createaccount_som.png` is no longer on the slide) | `talk/real_capture.py capture`, ONE session of the real Playwright MCP server (`@playwright/mcp` 0.0.81, server 1.64.0-alpha-2026-09-14) on en.wikipedia.org `Special:CreateAccount` (redirects to auth.wikimedia.org), viewport 1280×800, 15 Sep 2026 19:00 BST: `browser_snapshot` → `talk/fig/real_wiki_createaccount_snapshot.md` (161 YAML lines, same count as the 16:38 capture; `[ref=…]` ids differ), `browser_take_screenshot` → `real_wiki_createaccount_screenshot.png` (byte-identical to the 16:38 one), `browser_evaluate` → `real_wiki_createaccount_boxes.json` (25 interactive elements in the viewport, page order, outer of nested). `real_capture.py marks` draws those 25 boxes on that screenshot with the project's own SoM helpers (`p79/experiment/som.py::_draw_label`, `#00BCD4` outline, width 2, numbered 1..25) → `real_wiki_createaccount_som.png` = the BOTH panel. `render` → `real_wiki_snapshot_excerpt_compact.png` (20 of the 161 lines verbatim, elisions counted) for the slide; the wide excerpt stays for reference | one page, one session; the page changes. Lines, not tokens. None of the three is a Claude Code, Codex or Astra run; the frames name tools, not limits (the foot says so). The BOTH marks are ours, drawn on the real screenshot — say "the marks are ours", never that a product shows this. The Astra side rests on OpenAI's computer-use docs: the model "uses screenshots and other tool results to decide what to do next" |
| `27 → 43` solved /100 · `100% → 80%` relative cost (**the `hindsight` desk slide was removed 09-15, D22**; kept for Q&A) · `16` more solved · `20%` less cost (talk `hindsight`, shopping hook) | B0·classifieds from `talk/hindsight_efficiency.json`: best fixed view 27.23% → perfect hindsight 43.30%, +16.07 percentage points; cost −20.2%, hence 79.8% of fixed-view cost. Display values rounded | aggregate over 224 tasks; the desk-shopping request is a relatable analogy, not a recorded task or a Claude measurement. Both gains use the same setting and best-fixed comparator. No claim of purchase completion; not a relative 16% SR increase |
| `0 of 8` (talk `learned`, figure caption) | learned pickers that beat always-cheapest on both success and cost. The talk figure (09-15) plots only the held-out learned choice (orange dots) and the perfect router (purple squares); the in-sample "tested on its own training tasks" triangles are dropped from the slide. Corner note *every learned router costs more than the star*: all 8 held-out points have cost ratio > 1 (asserted in `talk_figures.py::routing`) | always paired with "even perfect hindsight: 1 of 8". Don't say "buys success by spending more" of all 8 — one held-out point solves fewer. The in-sample triangles are for Q&A only |
| `2–4×` · two of six above the line (talk `why`, `not-yet`) | the `2.1–4.2×` row above, rounded for the slide. The `why` figure plots the six VWA settings from `router_undersampling_control.json` `whichmode_scale`: x = solvable %, y = examples of the second most common right view, line = 12.5 (two classes each keep ≥10 training rows in a 5-fold split). Above: B0·classifieds 24, B1·classifieds 14; below: B0·reddit 6, B2·classifieds 5, B1·reddit 4, B2·reddit 3 | keep "at least". The arrow is a direction, **never** a crossing point ("solve 30% and you're over" is not licensed). Crossing the line = a choice can be trained and tested, **not** that it beats always-cheapest (hindsight: 1 of 8). "Scaling law" is a label, not a fitted law; the multiplier assumes today's mix of right views ⇒ a lower bound |
...
| "the oracle", "modes", "pp" (to a non-researcher) | "perfect router" in the talk (09-15, ROADMAP D21; the poster says "hindsight oracle", so "perfect hindsight" at the board), "a learned router" / "a learned choice" (the demo's label), "views", "more tasks in 100" |
