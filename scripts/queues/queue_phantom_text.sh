...
#   - baseline:  B0 | B1 | B2 | B3 | B4 | B5 | B3 | B4 | B5
...
acquire_site_lock "${SITE}" "${BENCHMARK}" "queue_phantom_text" || exit $?
...
if pgrep -f "run_experiment.py.*${RUN_ID}" > /dev/null; then
  ...
  if [[ "${RESET_BEFORE:-0}" == "1" ]] && wa_reset_supported "${BENCHMARK}" "${SITE}"; then
    reset_and_auth_gate --site "${SITE}" --repo "${REPO_DIR}" --python "${PYTHON_BIN}" --log-prefix "phantom_text" --reset-label "phantom_text_${SITE}" --benchmark "${BENCHMARK}"
    ...
    echo "[phantom_text][error] RESET_BEFORE=1 but no reset implementation for benchmark=${BENCHMARK} site=${SITE}." >&2
    echo "[phantom_text][error] Supported: all VWA sites; WA reddit / shopping / shopping_admin (B-1930, shared containers)." >&2
    exit 1
  else
    ...
    auth_only_gate --site "${SITE}" --repo "${REPO_DIR}" \
      --python "${PYTHON_BIN}" --log-prefix "phantom_text" \
...
# exited 0 and the caller read it as success. `|| exit $?` evaluates
# `$?` when exit runs, where it still holds the real rc (78 = contention).
acquire_watchdog_lock "${RUN_ID}" "queue_phantom_text" || exit $?
