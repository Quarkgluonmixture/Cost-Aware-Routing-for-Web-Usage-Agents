...
" 2>&1; then
        ...
        echo "[phantom_text][warn] reset failed (rc=$?); continuing anyway" >&2
      fi
    else
      echo "[phantom_text][warn] reset_vwa_sites.sh not found; skipping reset" >&2
    fi
...
if pgrep -f "experiment_watchdog.*${RUN_ID}" > /dev/null; then
  ...
  echo "[phantom_text] launching watchdog → ${WD_LOG}"
