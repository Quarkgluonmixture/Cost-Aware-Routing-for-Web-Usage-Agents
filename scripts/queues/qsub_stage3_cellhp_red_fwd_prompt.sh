#!/bin/bash -l
# Cell H-prompt-reddit: reddit forward × strong × som → phantom_prompt × N=24
# Stage 3 mechanism attribution mirror of cellhp_cls on reddit.
...
#$ -N cellhp_red
#$ -o /home/ucab352/Scratch/p79/logs/qsub_cellhp_red.$JOB_ID.out
#$ -e /home/ucab352/Scratch/p79/logs/qsub_cellhp_red.$JOB_ID.err
#$ -j n
...
OUT_DIR="$REPO_DIR/results/mechanistic/stage3_cellhp_red_fwd_prompt_myriad"
mkdir -p "$OUT_DIR"

echo "[$(date '+%H:%M:%S')] Cell H-prompt-reddit: reddit fwd × strong × som → phantom_prompt × N=24"
...
echo "[$(date '+%H:%M:%S')] Cell H-prompt-reddit DONE"
ls -la "$OUT_DIR/"
