#!/bin/bash -l
# Cell H-prompt-red-RAND: reddit forward × strong × P-SoM → P-text × N=24 ×
# random-injection negative control (Exp 5 axis-2 content-specificity check).
# Cross-site replication of qsub_stage3_cellhprompt_cls_rand.sh.
...
#$ -N cellhprm_red_rand
#$ -o /home/ucab352/Scratch/p79/logs/qsub_cellhprm_red_rand.$JOB_ID.out
#$ -e /home/ucab352/Scratch/p79/logs/qsub_cellhprm_red_rand.$JOB_ID.err
#$ -j n
...
OUT_DIR="$REPO_DIR/results/mechanistic/stage3_cellhprompt_red_fwd_ptext_rand_myriad"
mkdir -p "$OUT_DIR"

echo "[$(date '+%H:%M:%S')] Cell H-prompt-red-RAND: red fwd × strong × P-SoM → P-text × random-inject × N=24"
...
echo "[$(date '+%H:%M:%S')] Cell H-prompt-red-RAND DONE"
ls -la "$OUT_DIR/"
