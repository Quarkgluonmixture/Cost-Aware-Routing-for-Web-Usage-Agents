#!/bin/bash -l
# Cell H-prompt-red: reddit forward × strong × source=P-SoM × target=P-text × N=24.
# Exp 5 cross-site replication of H-prompt-cls. Isolates axis-2 prompt-family
# mechanism on reddit (text format held fixed at flat [SOM_MARKS]).
...
#$ -N cellhprm_red
#$ -o /home/ucab352/Scratch/p79/logs/qsub_cellhprm_red.$JOB_ID.out
#$ -e /home/ucab352/Scratch/p79/logs/qsub_cellhprm_red.$JOB_ID.err
#$ -j n
...
OUT_DIR="$REPO_DIR/results/mechanistic/stage3_cellhprompt_red_fwd_ptext_myriad"
mkdir -p "$OUT_DIR"

echo "[$(date '+%H:%M:%S')] Cell H-prompt-red: reddit fwd × strong × P-SoM → P-text × N=24"
...
echo "[$(date '+%H:%M:%S')] Cell H-prompt-red DONE"
ls -la "$OUT_DIR/"
