#!/bin/bash -l
# Cell H-text-reddit: reddit forward × strong × som → phantom_text × N=24
# Stage 3 mechanism attribution mirror of cellht_cls on reddit.
...
#$ -N cellht_red
#$ -o /home/ucab352/Scratch/p79/logs/qsub_cellht_red.$JOB_ID.out
#$ -e /home/ucab352/Scratch/p79/logs/qsub_cellht_red.$JOB_ID.err
#$ -j n
...
OUT_DIR="$REPO_DIR/results/mechanistic/stage3_cellht_red_fwd_text_myriad"
mkdir -p "$OUT_DIR"

echo "[$(date '+%H:%M:%S')] Cell H-text-reddit: reddit fwd × strong × som → phantom_text × N=24"
...
echo "[$(date '+%H:%M:%S')] Cell H-text-reddit DONE"
ls -la "$OUT_DIR/"
