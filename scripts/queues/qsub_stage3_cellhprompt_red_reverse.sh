#!/bin/bash -l
# Cell H-prompt-red-REVERSE: red reverse direction × strong × P-text → P-SoM ×
# N=24 (asymmetry test of axis-2 cellhprompt patching).
#
# Original cellhprompt_red (359511) tested forward: P-SoM (clean source, with
...
#$ -N cellhprm_red_rev
#$ -o /home/ucab352/Scratch/p79/logs/qsub_cellhprm_red_rev.$JOB_ID.out
#$ -e /home/ucab352/Scratch/p79/logs/qsub_cellhprm_red_rev.$JOB_ID.err
#$ -j n
...
OUT_DIR="$REPO_DIR/results/mechanistic/stage3_cellhprompt_red_rev_psom_myriad"
mkdir -p "$OUT_DIR"

echo "[$(date '+%H:%M:%S')] Cell H-prompt-red-REVERSE: P-text (no image) → P-SoM (clean) × N=24"
...
echo "[$(date '+%H:%M:%S')] Cell H-prompt-red-REVERSE DONE"
ls -la "$OUT_DIR/"
