...
# --- WebArena reddit -----------------------------------------------------------------
# Added 2026-08-03. The audit that prompted it: every routing product in this project is
# VWA-only, and the per-task routing SPACE is largest on `wa_B0` (36/104 tasks have more
# than one solver, against 68/224, 36/203, 29/224, 17/203, 4/224, 3/203 on the six VWA
# cells) with the largest oracle headroom (+16.35pp). So the negative result "routing does
# not pay" was established exactly where routing has the least to work with, and never
# tested where it has the most.
#
# The three router products that stay VWA-only genuinely need `extract_50_features`, whose
# paths are VWA constants. THESE policies do not: `triage_only` needs a binary
# solvable/not label (defined on all 104 WA tasks) and `route_only` needs the which-mode
# label, both read off the same episode summaries as the VWA cells. Nothing here is
# learned, so no feature table is involved.
...
def _wa_matrix(baseline: str) -> tuple[list[int], dict, dict] | None:
    """WA has no AMENDMENT exclusion list; its universe is the six-mode intersection
    (104), the same convention `aggregate_fusion_premium.load_wa` uses."""
    succ: dict[str, dict[int, bool]] = {}
    cost: dict[str, dict[int, float]] = {}
    for m, stem in WA_STEM.items():
        ...
        if len(hits) != 1:
            return None
        s_map, c_map = {}, {}
        files = (list(hits[0].glob("*/episodes/*summary*.json"))
        ...
        for f in files:
            ...
            if rec.get("sr_excluded"):
                continue
            t = int(rec["task_id"])
            s_map[t] = rec.get("success") is True
            c = rec.get(COST_FIELD)
            c_map[t] = float(c) if isinstance(c, (int, float)) else float("nan")
        if not s_map:
            return None
        succ[m], cost[m] = s_map, c_map
    common = sorted(set.intersection(*(set(v) for v in succ.values())))
    if not common:
        return None
    return common, succ, cost
...
def evaluate(cell: dict) -> dict | None:
    got = _wa_matrix(cell["baseline"]) if cell.get("_wa") else _cell_matrix(cell)
    if got is None:
        return None
    tasks, succ, cost = got
    n = len(tasks)
...
def main() -> int:
    ...
    results = [r for r in (evaluate(c) for c in list(CELLS) + WA_CELLS) if r]
    ...
    # --- cross-cell summary -----------------------------------------------------------
    # Added with the WA cells 2026-08-03. The point of the table is the last two columns
    # read together: routing SPACE (share of tasks with >1 solver) and routing SUPPLY
    # (how many rows that actually is). They move in opposite directions across the grid.
    L += ["\n## Across cells\n",
          "| cell | n | best single | `triage_only` ΔSR | Δcost | `route_only` ΔSR "
          "| >1 solver | share | no solver |",
          "|---|---|---|---|---|---|---|---|---|"]
    tri_dcosts, rows_supply = [], []
    for r in results:
        p = r["policies"]; b = p["best_sr"]
        tri, rte, un = p["triage_only"], p["route_only"], p["_unsolvable"]
        n, ms = r["n_tasks"], p["oracle_sr_cost"]["n_multi_solver_tasks"]
        dc = 100.0 * (tri["mean_cost"] / b["mean_cost"] - 1)
        tri_dcosts.append(dc)
        rows_supply.append((f"{r['site']}·{r['baseline']}", ms, 100.0 * ms / n))
        L.append(
            f"| `{r['site']}·{r['baseline']}` | {n} | {b['sr_pct']:.2f}% "
            f"| {tri['sr_pct'] - b['sr_pct']:+.2f}pp | {dc:+.1f}% "
            f"| {rte['sr_pct'] - b['sr_pct']:+.2f}pp | {ms} | {100.0*ms/n:.1f}% "
            f"| {un['pct']:.1f}% |")
    _lo, _hi = min(tri_dcosts), max(tri_dcosts)
    _dense = max(rows_supply, key=lambda r: r[2])
    _sparse = min(rows_supply, key=lambda r: r[2])
    L += [
        f"\n- **`triage_only` is free in {len(results)} of {len(results)} cells**: zero SR "
        f"change by construction, {_hi:+.1f}% to {_lo:+.1f}% cost. Its label is binary and "
        "defined on every task, so its supply never binds.",
        f"- **Routing space and routing supply move in opposite directions.** `{_dense[0]}` "
        f"has the densest per-task routing space at {_dense[2]:.1f}% of tasks "
        f"({_dense[1]} rows) and `{_sparse[0]}` the sparsest at {_sparse[2]:.1f}% "
        f"({_sparse[1]} rows) — but in *absolute* rows the densest cell supplies "
        f"{_dense[1]}, which is the same order as the mid-grid cells. A bigger share of a "
        "smaller benchmark is not more supervision.",
        "- ⚠️ Everything above is an **oracle**: it says what a perfect chooser would have "
        "gotten on runs that already landed. `router_triage_learnability` is the "
        "out-of-fold counterpart and stays VWA-only, because it needs "
        "`extract_50_features`, whose paths are VWA constants. So the WA rows here extend "
        "the *upper bound* to both benchmarks; they do not extend the learned result.",
    ]
