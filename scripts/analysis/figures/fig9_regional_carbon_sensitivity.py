...
def main() -> None:
    ...
    pending_handles = [
        Line2D([0], [0], color=COLORS["Phantom-SoM"], linestyle=":", lw=2, label="Phantom-SoM pending"),
        Line2D([0], [0], color=COLORS["Phantom-DOM"], linestyle=":", lw=2, label="Phantom-DOM pending"),
    ]
    handles, labels = axes[0].get_legend_handles_labels()
    axes[1].legend(handles + pending_handles, labels + [h.get_label() for h in pending_handles], loc="upper right", fontsize=8.5)

    fig.suptitle("Regional Carbon Sensitivity from B1 Measured Energy", fontsize=15, fontweight="bold")
    fig.text(
        0.5,
        0.02,
        "Energy from B1 (Qwen3-VL-4B) NVML measurement on cls (N=234) / red (N=210). "
        "Per-region intensity from IEA 2023 / ElectricityMaps. Phantom-SoM/Phantom-DOM pending re-run; expected to be comparable or lower than DOM.",
        ha="center",
        fontsize=8.2,
        color="#555555",
    )
