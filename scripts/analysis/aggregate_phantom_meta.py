...
def main() -> int:
    ...
    lines = [
        ...
        "",
        "> **⚠️ APPENDIX-ONLY (B-1016 /stress A2.4a P1-8-AB*, 2026-05-18)**: This producer",
        "> uses **N_common ≥ 10 inclusion lock** (B8 lock 2026-05-09) for axis-pair universe",
        "> definition. It is **NOT** the canonical paper §1 hero gate producer. For the",
        "> canonical H1 + H2(a) + H3 + R1-R5 verdict use",
        "> `scripts/analysis/aggregate_phase1_full_prereg_decision.py` (B-515 A1.21) which",
        "> emits to `results/phantom_paper/phase1_full_prereg_decision.{csv,json,md}` and",
        "> uses the **six-arm complete-case universe** (B-948 A2.3a Chunk 3) matching",
        "> `aggregate_phantom_lift.py:797-810`. The two universe semantics may produce",
        "> different H3 unique-count numbers post-Phase-1a-fire — the canonical producer",
        "> is the paper-grade gate; this meta is appendix-only DL random-effects sensitivity.",
        "> See preregistration.md §2 H3 + 笔记 §177 + ## A2.4a master_bug_catalog section.",
        "",
        ...
        "**Family scoping clarification (B-182; B-185 stale-status line retired",
        "B-1875 2026-06-09)**: this table reports RE meta-pooled",
        "estimates per arm. The PRIMARY paper gate is the FE one-sided superiority",
        "test (H0: θ_FE ≤ +1.0pp) computed separately by the canonical producer",
        "`aggregate_phase1_full_prereg_decision.py` → `phase1_full_prereg_decision.{csv,json,md}`",
        "(+ `aggregate_phase1_prereg_gate.py` companion). The Holm-corrected",
    ...
    lines += [
        ...
        "## Notes",
        "",
        "- **Family-wise correction as implemented (B-1875 prose↔code alignment)**:",
        "  Holm step-down is applied WITHIN each family by this script — PRIMARY m=1",
        "  (degenerate, p_re_holm = p_re), SECONDARY m=3, TERTIARY m=2. TERTIARY",
        "  remains gating_status=exploratory (appendix sensitivity, never a gate);",
        "  its p_re_holm column is reported for transparency, not inference.",
