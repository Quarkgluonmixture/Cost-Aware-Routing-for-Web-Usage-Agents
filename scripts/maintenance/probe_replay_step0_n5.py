#!/usr/bin/env python3
"""B-1867 / §302 follow-up: N=5 same-payload replay to discriminate codex top-2.

Sends the SAME production-like vision payload N=5 back-to-back through the
AWS proxy and compares responses. Codex §5 minimal next experiment:

  - 5/5 unique within minutes  → remote/tool-decoder nondet (#1+#3 alive)
  - 5/5 identical within mins, 24h drift → alias/deployment drift (#2)
  - all deterministic both windows → reconstruction missed a field

NOT a GPU experiment. Sends HTTPS POST to AWS API Gateway → Bedrock.
DGX-safe; does NOT compete with A100 fire (separate outbound).

Output: docs/checkpoints/probes/replay_step0_n5_<HHMMSS>.json
"""
...
def call_once(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "resp_headers": {
            ...
            if k.lower() in ("x-amzn-requestid", "x-amz-request-id", "x-amzn-trace-id",
                             "x-amz-cf-id", "via", "server", "date", "content-length")
        },
    }
    try:
        body = r.json()
        ...
        if isinstance(tc, list) and tc:
            ...
            if isinstance(fn, dict):
                ...
                try:
                    ...
                    out["args"] = args
                    out["action_type"] = args.get("action_type")
                    out["element_id"] = args.get("element_id")
                    out["coord"] = args.get("coordinate")
                    out["thought_snip"] = (args.get("thought") or "")[:200]
                except json.JSONDecodeError:
...
def run_task(task_id: int, n: int, sleep_s: float = 0.0) -> Dict[str, Any]:
    ...
    for i in range(n):
        ...
        rid = c.get("resp_headers", {}).get("x-amzn-requestid", "?")[:12]
        print(f"  [{i+1}/{n}] status={c.get('status')} ms={c.get('elapsed_ms')} action={sig} margin={m} reqid={rid}")
        calls.append(c)
...
def main() -> int:
    ...
    print(f"§302 N=5 same-payload replay  tasks={task_ids}  N={args.n}  sleep={args.sleep}s")
    ...
    summary = {
        ...
        "purpose": "B-1867/§302 N=5 same-payload replay — discriminate remote-serving vs alias-drift",
        "n_per_task": args.n,
        "task_ids": task_ids,
        "wall_seconds": wall_s,
        "results": results,
    ...
    out_path = out_dir / f"replay_step0_n5_{hhmmss}{label}.json"
