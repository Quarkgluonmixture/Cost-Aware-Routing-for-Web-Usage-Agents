...
run_site_3mode_with_reset() {
  ...
  refresh_site_auth "${site}" || {
    log "[b1_wa][error] ${site} SOM 前 auth 刷新失败，等待 30s 后重试..."
    sleep 30
    refresh_site_auth "${site}" || { log "[b1_wa][fatal] ${site} auth 刷新两次均失败，中止"; exit 1; }
  }
  ...
  sleep 10
  refresh_site_auth "${site}" || {
    log "[b1_wa][error] ${site} Vision 前 auth 刷新失败，等待 30s 后重试..."
    sleep 30
    refresh_site_auth "${site}" || { log "[b1_wa][fatal] ${site} auth 刷新两次均失败，中止"; exit 1; }
  }
