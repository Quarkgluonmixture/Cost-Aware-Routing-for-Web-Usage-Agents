...
#
# Gallery: http://localhost:8765/B1_3mode/gallery.html
set -euo pipefail
...
start_gallery_server() {
  if ! ss -tlnp 2>/dev/null | grep -q ':8765 '; then
    ...
    kill -0 "${GALLERY_PID}" 2>/dev/null \
      && log "[b1_3mode] Gallery pid=${GALLERY_PID} url=http://localhost:8765/${AGGREGATE_PREFIX}/gallery.html" \
...
run_site_3mode_with_reset() {
  ...
  refresh_site_auth "${site}" || {
    log "[b1_3mode][error] ${site} SOM 前 auth 刷新失败，等待 30s 后重试..."
    sleep 30
    refresh_site_auth "${site}" || { log "[b1_3mode][fatal] ${site} auth 刷新两次均失败，中止"; exit 1; }
  }
  ...
  sleep 10
  refresh_site_auth "${site}" || {
    log "[b1_3mode][error] ${site} Vision 前 auth 刷新失败，等待 30s 后重试..."
    sleep 30
    refresh_site_auth "${site}" || { log "[b1_3mode][fatal] ${site} auth 刷新两次均失败，中止"; exit 1; }
  }
...
log "=== 顺序: reddit → shopping（不含 classifieds，B0 已覆盖）==="
...
log "========================================================"
log "=== B1 三模式全部完成（reddit + shopping）==="
log "=== Gallery: http://localhost:8765/${AGGREGATE_PREFIX}/gallery.html ==="
log "========================================================"
ntfy_send "P79 [B1_3mode] 全部完成!" "reddit + shopping 三模式全部跑完" "high"
