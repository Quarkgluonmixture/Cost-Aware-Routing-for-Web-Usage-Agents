...
|---|---|---|---|---|---|---|---|
| **C1** | 表征之间真的有值得路由的差异吗？ | **上限真实存在**：oracle 相对最强单模 SR **+3.45~16.35pp**，成本低 **13.7–35.3%** | `router_objective_ordering.md`（`oracle_sr_cost` 行）· `phase1_full_prereg_decision.json` · `sr_per_mode.json` | **8 cells 一致方向，跨两个 benchmark**（6 VWA + 2 WA）；FE inverse-variance pooled | oracle 是**事后**上界，非可达策略；FE 估计量只说这些格。⚠️ **2026-08-10 追加（`noise_floor_inventory.md` §3 ①）**：该 5-arm 增益 **survives**，但**headline 必须把重跑基线印在旁边**——*"one rerun buys 2.0–7.6pp"*。即：16.07pp 里有约 2–7.6pp 是"再跑一次也能买到的"。**这是全文唯一还站着的正面主张，所以这条限定不能省** | Ch4 | A |
...
| **C2** | 这个上限有结构基础，还是只是随机涨落？ | ⚠️ **降级（2026-08-10 按 `noise_floor_inventory.md` §3 重判）**：三个 phantom arm 各自独解一批任务（drop-one oracle **0.00–2.68pp**，**14/18 arm-cell 为正**——⚠️ **2026-08-10 按 08-10 六格产物重算，原写「1.7–3.3pp / 22/24」是旧口径**，新数字**更弱**：P-prompt 只有 4/6 为正），但**这不构成正面的结构性主张** —— pooled 双轴效应 **1.35 / 2.09pp** 低于**最宽松的已测重跑地板 2.0pp**。原文判词：*"does not survive as a positive claim"* | **`fig0c_drop_one_bootstrap_ci.csv`（08-10，6 cells × 6 modes，complete-case，⚠️ grade=NON_PAPER_GRADE）** · `phantom_lift.csv`（08-10，6 格）· **`noise_floor_inventory.md` §2§3**（08-04）。⚠️ ~~`meta_phantom_lift.csv`~~ **不要再引**：mtime 05-17 且 **k_cells=3**，自身标 `appendix-only` | **14/18** phantom arm-cell drop-one 为正（全 6 mode 则 32/36）—— ⚠️ 但方向一致 ≠ 幅度超噪声：**36 个值里只有 3 个**达到 3.82pp 的过噪声门槛，且**全部是 baseline mode，无一 phantom** | ⚠️ **H3 过门是弱证据**（§397.8）+ **已于 2026-07-28 降级**；现进一步：在 `B0·VWA-cls` 上，**加一个不同表征的 arm（7.14pp）与加一次同 arm 重跑（4.91–7.59pp）无法区分**。C2 只能写成"观察到方向一致的互补结构，但其幅度未超过重复采样地板" | Ch4 | A |
| **C3** | 这个结构比"什么都不做重跑一次"更大吗？ | **不**——结构量级落在同模式重跑的噪声地板内 | `compare_cross_run_same_condition.py`：discordance **14.3%**（32/224），Cohen κ **0.614** | self-oracle drop-one A→B 6.7pp / B→A 7.6pp；**2026-08-10 更新：`B0·VWA-cls` 已有 3 个 replicated arm** (dom **12.05%** / vision **14.29%** / som **12.95%** discordance)，som 于 08-03 落地 ⇒ C3 不再是外推 | ⚠️ self-oracle drop 只能当 **instability diagnostic**，**不是 bias estimate**（§293）；净差 +0.9pp 不是噪声地板本身。⚠️ Cohen κ **0.614** 只有 `vision` 那一对算过；另两对目前只有 discordance | Ch4 末 / Ch6 | B |
| **C4** | 那能不能学一个便宜的预测器？ | **不能**：真嵌套 CV 下 **0/8 cell** 的 learned triage 能 Pareto 胜过平凡的 always-cheapest（**跨两个 benchmark**，2026-08-10 更正，原写 0/6） | `router_triage_learnability_with_wa.md:124`（8 格匹配 18 特征集）· `router_triage_learnability.md`（§392.2 真嵌套版, VWA 20 特征） | bundle-permutation 零分布（**B=10000**，2026-08-10 更正，原写 B=200）+ always-cheapest 固定策略，**两道控制缺一不可** | 权衡点而非压制点（**8 格口径，2026-08-10 逐格重算**）：最贵的 `wa_reddit·B1` **+6.73pp 换 +41.7% cost**、`cls·B1` **+1.79pp 换 +36.1%**；最便宜的 `red·B2` **+2.46pp 换 +1.7%**。⚠️ 原写"cls·B0 +1.79pp 但 cost +11%"是**串行**——+1.79pp 属 cls·B1，+11% 接近 red·B0 的 +11.6%，而 cls·B1 真实 cost 是 +36.1%（6 格版 +36.5%）。修正后**更支持**"不是压制点"。<br>🔴 **2026-08-10 画 F13 时发现的自我限定（必须写进正文，否则 examiner 必问）**：把每格归一到 always-cheapest 后，**连事后 `oracle_triage` 也只有 1/8 落进 Pareto 胜区**。原因是 always-cheapest 是**成本下界**——任何保住 SR 的策略都必然更贵。⇒ C4 的负结果**部分是对照选择的产物**，正确表述是"**这个权衡不划算**"，而**不是**"学习器不行"。这不削弱 C4（0/8 仍成立），但把它从"learner 失败"校准为"**在该成本口径下不存在划算的可部署点**" | Ch6 | C |
...
|---|---|---|---|
| ✅ ~~**同模式重跑的噪声地板只有 vision 一格**~~ **已过时（2026-08-10 核出，第四次"缺口比产物旧"）** | C3 | `noise_floor_inventory.md:74` 明写 **`B0·VWA-cls` 现在有 3 个 replicated arm（dom / vision / som）**，`som` pair **08-03 落地**；另有 `B1·WA-red` 一格（⚠️ 但它 pooled 5 modes × 10 tasks，不能当自身 baseline arm 的地板） | **不必再限定到"那一格"**。而且 `som` 那格是最要紧的一个——原文：*"claim 3 is about the fused arm, and until that day its floor was **borrowed** from DOM and Vision"*。结果是 **borrowed band 被证明是对的**（som 自身 5.36–7.59pp 落在其中，mean-difference draw 2.23pp 与 DOM 相同）⇒ *"a robustness result rather than a correction … the claim no longer rests on an extrapolation"*。**下游数字一个都没动，但 C3 的证据基础从外推变成实测** |
...
| **A2** | **drop-one 1.7–3.3pp 可能只是 pass@K** —— C3 的噪声地板 discordance **14.3%** 比它大一个量级。跑同一个最强 arm 两次也会翻掉一些失败 | 若成立，C2 的"双轴独立"直接崩 | ✅ **已做，2026-08-04 `noise_floor_inventory.md`**（2026-08-10 才发现——本行原写"未做，2–3h"，且台账 `pass@` 0 match 是**假阴性**：该分析没用 pass@k 这个词。又一次缺口比产物旧）。它做的正是 A2 要求的 one-arm-margin 对照 `\|{added} ∖ {baseline}\| / n`——**加一个异表征 arm 与加一次同 arm 重跑，在形式上是同一个量** | 🔴 **攻击成立**。`B0·VWA-cls`：+1 最佳异表征 **7.14pp** vs +1 重跑 **4.91–7.59pp** ⇒ 原文判词 *"indistinguishable — inside the rerun band"* / *"Neither cell shows a representation arm worth appreciably more than a rerun arm; one shows it worth no more at all."* ⚠️ 且 7.14pp 是 **best** 异表征，phantom arm 贡献更小 ⇒ **攻击只会更强**。<br>⚠️ 剩下的真 gap：该对照是 **1-arm baseline +1** 口径，C2 的 drop-one 是 **6-arm oracle −1** 口径，inventory 明确拒绝跨口径比较（*"the 6-mode ceiling gain is NOT comparable to a one-rerun floor"*）⇒ 6-arm 口径的对照**做不了**：只有 1 个重跑 arm，凑不出六臂 |
