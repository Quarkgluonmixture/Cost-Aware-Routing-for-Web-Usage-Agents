...
| **C2** | 这个上限有结构基础，还是只是随机涨落？ | ⚠️ **降级（2026-08-10 按 `noise_floor_inventory.md` §3 重判）**：三个 phantom arm 各自独解一批任务（drop-one oracle **1.7–3.3pp**，22/24 arm 为正），但**这不构成正面的结构性主张** —— pooled 双轴效应 **1.35 / 2.09pp** 低于**最宽松的已测重跑地板 2.0pp**。原文判词：*"does not survive as a positive claim"* | `meta_phantom_lift.csv`（⚠️ mtime 05-17）· drop-one oracle 表 · **`noise_floor_inventory.md` §2§3**（08-04） | 22/24 arm 观测 drop-one 为正 —— ⚠️ 但方向一致 ≠ 幅度超噪声 | ⚠️ **H3 过门是弱证据**（§397.8）+ **已于 2026-07-28 降级**；现进一步：在 `B0·VWA-cls` 上，**加一个不同表征的 arm（7.14pp）与加一次同 arm 重跑（4.91–7.59pp）无法区分**。C2 只能写成"观察到方向一致的互补结构，但其幅度未超过重复采样地板" | Ch4 | A |
