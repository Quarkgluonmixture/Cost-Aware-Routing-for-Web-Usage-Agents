...
|---|---|---|---|---|
| **A1** | **C4 的"结构性"可能只是欠采样** —— C5 自己说 4/6 cell 只有 15–97 个可训练标签。用 15 个正例在嵌套 CV 里学不出 Pareto 最优, 是欠采样的平凡后果 | **直击 headline**。它把一个负结果降级成"你数据太少" —— 而这恰好是最便宜的驳回理由 | 标签最多那格的**学习曲线**; 或证明**无正则化模型过拟合训练集仍分不开** | ✅ **已做, 2026-08-11** `router_undersampling_control.{md,json}` + F16。**攻击对预测器成立, 对结论不成立**: (a) 学习曲线仍在涨 (+0.032~+0.083 AUROC), 但 12 条里 9 条在减速; (b) 无正则化 in-sample 超过置换地板 +0.075~+0.116 (p≤0.01 ×5), **特征确有信号** ⇒ 不能用"没东西可学"打发; (c) **决定性**: 更多数据把预测器推向 oracle, 而 oracle 只在 **1/8 格**进胜区 ⇒ 欠采样能解释 0.73→1.00 的差距, **解释不了 oracle 自己也过不去的那条边界**。顺带把 supply 约束**定价**: 四个失败格需 **2.1–4.2×** 语料 (reddit 最多 846 题), 是**规格**不是不可能 —— 这实际**削弱**了原文"structural"的措辞, 已在 Ch7 改口 |
