...
combinations. The idealised version of the \emph{same} policy, told in advance
whether any mode will solve the task, fails to clear it in 7 of 8. That recasts
the negative result from ``the learner failed'' to ``no Pareto-improving
\emph{deployable} operating point of this kind exists against always-cheapest
under this cost boundary''. A chooser told \emph{which} modes solved each task
is a different and stronger policy, and it does clear the bar in 7 of 8; it is
also not implementable, because it needs the answer the next section shows the
benchmarks cannot teach.
...
predictor, but the best a \emph{triage} predictor can become is the triage
oracle, which wins in 1 of 8 combinations. Pricing this constraint puts the benchmark that
