...
\emph{Referenced from Section~\ref{sec:ceiling}.} The main text reports the
ceiling as two summary columns. This appendix gives the full policy ladder per
cell, because the summary hides two quantities a reader may want to check
independently: how far each individual fixed mode sits from the best one, and
how the oracle splits between its triage and which-mode halves.
...
\begin{table}[h]
\centering\small
\caption{Full policy ladder, \cell{cls$\cdot$B0} ($n=224$) and
\cell{red$\cdot$B0} ($n=203$). $\Delta$ columns are against the best single
...
\toprule
& & \multicolumn{2}{c}{\cell{cls$\cdot$B0}} & \multicolumn{2}{c}{\cell{red$\cdot$B0}} \\
